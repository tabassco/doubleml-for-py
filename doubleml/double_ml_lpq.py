...
class DoubleMLLPQ(NonLinearScoreMixin, DoubleML):
    ...
    def __init__(
        ...
        if kde is None:
            ...
            if not callable(kde):
                raise TypeError(
                    "kde should be either a callable or None. " "%r was passed." % kde
                )
            self._kde = kde
        ...
        if not isinstance(self.normalize_ipw, bool):
            raise TypeError(
                "Normalization indicator has to be boolean. "
        ...
        self._coef_start_val = np.quantile(
            self._dml_data.y[self._dml_data.d == self.treatment], self.quantile
        )
        ...
        if draw_sample_splitting:
            strata = self._dml_data.d.reshape(-1, 1) + 2 * self._dml_data.z.reshape(
                -1, 1
            )
    ...
    def _nuisance_est(self, smpls, n_jobs_cv, return_models=False):
        ...
        for learner in self.params_names:
            ...
            if est_params is not None:
                fitted_models[learner] = [
                    clone(self._learner[learner]).set_params(**est_params[i_fold])
                    for i_fold in range(self.n_folds)
                ]
            else:
                fitted_models[learner] = [
                    clone(self._learner[learner]) for i_fold in range(self.n_folds)
                ]
        ...
        for i_fold in range(self.n_folds):
            ...
            smpls_prelim = [
                (train, test)
                for train, test in StratifiedKFold(n_splits=self.n_folds).split(
                    X=train_inds_1, y=strata[train_inds_1]
                )
            ]
            ...
            m_z_hat_prelim = _trimm(
                m_z_hat_prelim, self.trimming_rule, self.trimming_threshold
            )
            ...
            m_d_z0_hat_prelim = _predict_zero_one_propensity(
                ml_m_d_z0_prelim, x_train_1
            )
            ...
            comp_prob_prelim = np.mean(
                ...
                - (1 - z_train_1)
                / (1 - m_z_hat_prelim)
                * (d_train_1 - m_d_z0_hat_prelim)
            )

            # preliminary ipw estimate
            def ipw_score(theta):
                res = np.mean(
                    self._compute_ipw_score(
                        theta,
                        d_train_1,
                        y_train_1,
                        m_z_hat_prelim,
            ...
            _, bracket_guess = _get_bracket_guess(
                ipw_score, self._coef_start_val, self._coef_bounds
            )
            ...
            du_z0_train_2 = (d_train_2[z0_train_2] == self._treatment) * (
                y_train_2[z0_train_2] <= ipw_est
            )
            fitted_models["ml_g_du_z0"][i_fold].fit(x_z0_train_2, du_z0_train_2)
            g_du_z0_hat["preds"][test_inds] = _predict_zero_one_propensity(
                fitted_models["ml_g_du_z0"][i_fold], x_test
            )

            # propensity for (D == treatment)*Ind(Y <= ipq_est) cond. on z == 1
            z1_train_2 = z_train_2 == 1
            x_z1_train_2 = x_train_2[z1_train_2, :]
            du_z1_train_2 = (d_train_2[z1_train_2] == self._treatment) * (
                y_train_2[z1_train_2] <= ipw_est
            )
            fitted_models["ml_g_du_z1"][i_fold].fit(x_z1_train_2, du_z1_train_2)
            g_du_z1_hat["preds"][test_inds] = _predict_zero_one_propensity(
                fitted_models["ml_g_du_z1"][i_fold], x_test
            )
            ...
            g_du_z0_hat["targets"][test_inds_z0] = (
                1.0
                * (d_test[z_test == 0] == self._treatment)
                * (y_test[z_test == 0] <= ipw_est)
            )
            g_du_z1_hat["targets"][test_inds_z1] = (
                1.0
                * (d_test[z_test == 1] == self._treatment)
            ...
            m_z_hat["preds"][test_inds] = _predict_zero_one_propensity(
                fitted_models["ml_m_z"][i_fold], x_test
            )
            ...
            m_d_z0_hat["preds"][test_inds] = _predict_zero_one_propensity(
                fitted_models["ml_m_d_z0"][i_fold], x_test
            )
            ...
            m_d_z1_hat["preds"][test_inds] = _predict_zero_one_propensity(
                fitted_models["ml_m_d_z1"][i_fold], x_test
            )
        ...
        # clip propensities
        m_z_hat_adj = _trimm(
            m_z_hat["preds"], self.trimming_rule, self.trimming_threshold
        )

        if self._normalize_ipw:
            if self.dml_procedure == "dml1":
                for _, test_index in smpls:
                    m_z_hat_adj[test_index] = _normalize_ipw(
                        m_z_hat_adj[test_index], z[test_index]
                    )
            else:
    ...
    def _nuisance_tuning(
        ...
        train_inds_z0 = [
            np.intersect1d(np.where(z == 0)[0], train) for train, _ in smpls
        ]
        train_inds_z1 = [
            np.intersect1d(np.where(z == 1)[0], train) for train, _ in smpls
        ]
    ...
    def _check_data(self, obj_dml_data):
        if not isinstance(obj_dml_data, DoubleMLData):
            raise TypeError(
                "The data must be of DoubleMLData type. "
