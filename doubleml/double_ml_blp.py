...
class DoubleMLBLP:
    ...
    def confint(self, basis=None, joint=False, level=0.95, n_rep_boot=500):
        ...
        if basis is None:
            ...
            raise ValueError(
                "Invalid basis: DataFrame has to have the exact same number and ordering of columns."
            )
        ...
        if joint:
            ...
            bootstrap_samples = np.multiply(
                np.dot(np_basis, np.dot(sqrtm(self._blp_omega), normal_samples)).T,
                (1.0 / blp_se),
            )
