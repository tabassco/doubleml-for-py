...
class DoubleMLQTE:
    ...
    def summary(self):
        ...
        if np.isnan(self.coef).all():
            ...
            df_summary = pd.DataFrame(
                summary_stats, columns=col_names, index=self.quantiles
            )
            ci = self.confint()
    ...
    def fit(
        ...
        fitted_models = parallel(
            delayed(self._fit_quantile)(
                i_quant, n_jobs_cv, store_predictions, store_models
            )
        ...
        for i_quant in range(self.n_quantiles):
            ...
            # treatment Effects
            self._all_coef[self._i_quant, :] = (
                self.modellist_1[self._i_quant].all_coef
                - self.modellist_0[self._i_quant].all_coef
            )

            # save scores and derivatives
            self._psi0[:, :, self._i_quant] = np.squeeze(
                self.modellist_0[i_quant].psi, 2
            )
            self._psi1[:, :, self._i_quant] = np.squeeze(
                self.modellist_1[i_quant].psi, 2
            )

            self._psi0_deriv[:, :, self._i_quant] = np.squeeze(
                self.modellist_0[i_quant].psi_deriv, 2
            )
            self._psi1_deriv[:, :, self._i_quant] = np.squeeze(
                self.modellist_1[i_quant].psi_deriv, 2
            )
    ...
    def bootstrap(self, method="normal", n_rep_boot=500):
        ...
        for i_rep in range(self.n_rep):
            ...
            for i_quant in range(self.n_quantiles):
                ...
                (
                    self._boot_coef[self._i_quant, i_start:i_end],
                    self._boot_t_stat[self._i_quant, i_start:i_end],
    ...
    def _compute_bootstrap(self, weights):
        if self.apply_cross_fitting:
            ...
            boot_t_stat = np.matmul(weights, scaled_score) / (
                self._dml_data.n_obs * self.__all_se
            )
        else:
            # be prepared for the case of test sets of different size in repeated no-cross-fitting
            smpls = self.__smpls
    ...
    def confint(self, joint=False, level=0.95):
        ...
        df_ci = pd.DataFrame(
            ci, columns=["{:.1f} %".format(i * 100) for i in ab], index=self._quantiles
        )
        return df_ci

    def _fit_quantile(
        self, i_quant, n_jobs_cv=None, store_predictions=True, store_models=False
    ):
    ...
    def _agg_cross_fit(self):
        ...
        self.se = np.sqrt(
            np.divide(
                np.median(
                    np.multiply(np.power(self._all_se, 2), self._var_scaling_factor)
                    + np.power(self._all_coef - xx, 2),
                    1,
                ),
                self._var_scaling_factor,
            )
        )
    ...
    def _initialize_arrays(self):
        ...
        psi0_deriv = np.full(
            (self._dml_data.n_obs, self.n_rep, self.n_quantiles), np.nan
        )
        ...
        if self.dml_procedure == "dml1":
            if self.apply_cross_fitting:
                all_dml1_coef = np.full(
                    (self.n_quantiles, self.n_rep, self.n_folds), np.nan
                )
            else:
    ...
    def _check_quantile(self):
        if np.any(self.quantiles <= 0) | np.any(self.quantiles >= 1):
            raise ValueError(
                "Quantiles have be between 0 or 1. "
