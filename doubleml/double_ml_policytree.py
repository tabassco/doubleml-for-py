    ...
    **tree_params : dict
    ...
    def __init__(self, orth_signal, features, depth=2, **tree_params):
        ...
        if not isinstance(features, pd.DataFrame):
            raise TypeError(
                "The features must be of DataFrame type. "
        ...
        # initialize tree
        self._policy_tree = DecisionTreeClassifier(
            max_depth=self._depth, **self._tree_params
        )
    ...
    def summary(self):
        ...
        summary = pd.DataFrame(
            {"Decision Variables": self._features.keys(), "Max Depth": self._depth}
        )
        return summary
