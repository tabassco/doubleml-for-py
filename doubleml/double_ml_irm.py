    ...
    **Interactive regression (IRM)** models take the form
    ...
    def _nuisance_est(self, smpls, n_jobs_cv, return_models=False):
        ...
        if self._dml_data.binary_outcome:
            ...
            zero_one_preds = np.all(
                (np.power(g_hat0["preds"], 2) - g_hat0["preds"]) == 0
            )
            if binary_preds & zero_one_preds:
                raise ValueError(
                    f"For the binary outcome variable {self._dml_data.y_col}, "
        ...
        if self._dml_data.binary_outcome:
            ...
            zero_one_preds = np.all(
                (np.power(g_hat1["preds"], 2) - g_hat1["preds"]) == 0
            )
            if binary_preds & zero_one_preds:
                raise ValueError(
                    f"For the binary outcome variable {self._dml_data.y_col}, "
        ...
        m_hat["preds"] = _trimm(
            m_hat["preds"], self.trimming_rule, self.trimming_threshold
        )

        psi_a, psi_b = self._score_elements(
            y, d, g_hat0["preds"], g_hat1["preds"], m_hat["preds"], smpls
        )
    ...
    def _score_elements(self, y, d, g_hat0, g_hat1, m_hat, smpls):
        ...
        if isinstance(self.score, str):
            ...
            psi_a, psi_b = self.score(
                y=y, d=d, g_hat0=g_hat0, g_hat1=g_hat1, m_hat=m_hat, smpls=smpls
            )
    ...
    def cate(self, basis):
        ...
        if self.n_rep != 1:
            raise NotImplementedError(
                "Only implemented for one repetition. "
    ...
    def gate(self, groups):
        ...
        if not isinstance(groups, pd.DataFrame):
            raise TypeError(
                "Groups must be of DataFrame type. "
        ...
        if any(groups.sum(0) <= 5):
            warnings.warn(
                "At least one group effect is estimated with less than 6 observations."
            )
    ...
    def policy_tree(self, features, depth=2, **tree_params):
        ...
        **tree_params : dict
        ...
        if not isinstance(features, pd.DataFrame):
            raise TypeError(
                "Covariates must be of DataFrame type. "
                f"Covariates of type {str(type(features))} was passed."
            )

        orth_signal = self.psi_elements["psi_b"].reshape(-1)

        model = DoubleMLPolicyTree(
            orth_signal, depth=depth, features=features, **tree_params
        ).fit()
