...
def fit_cvar(
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            cvars[i_rep], ses[i_rep] = cvar_dml1(
                y, d, g_hat, m_hat, treatment, quantile, smpls, ipw_est
            )
        else:
            cvars[i_rep], ses[i_rep] = cvar_dml2(
                y, d, g_hat, m_hat, treatment, quantile, ipw_est
            )

    cvar = np.median(cvars)
    se = np.sqrt(
        np.median(np.power(ses, 2) * n_obs + np.power(cvars - cvar, 2)) / n_obs
    )
...
def fit_nuisance_cvar(
    ...
    for i_fold, _ in enumerate(smpls):
        ...
        m_hat_prelim = _dml_cv_predict(
            ml_m, x_train_1, d_train_1, method="predict_proba", smpls=smpls_prelim
        )["preds"]
        ...
        def ipw_score(theta):
            res = np.mean(
                (d_train_1 == treatment) * (y_train_1 <= theta) / m_hat_prelim
                - quantile
            )
            return res
...
def cvar_dml1(y, d, g_hat, m_hat, treatment, quantile, smpls, ipw_est):
    ...
    se = np.sqrt(
        cvar_var_est(theta_hat, g_hat, m_hat, d, y, treatment, quantile, ipw_est, n_obs)
    )
