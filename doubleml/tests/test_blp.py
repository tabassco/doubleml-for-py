...
def dml_blp_fixture(ci_joint, ci_level):
    ...
    np.random.seed(42)
    ci_manual = blp_confint(
        blp_manual, random_basis, joint=ci_joint, level=ci_level, n_rep_boot=1000
    )
...
def test_dml_blp_coef(dml_blp_fixture):
    assert np.allclose(
        dml_blp_fixture["coef"], dml_blp_fixture["coef_manual"], rtol=1e-9, atol=1e-4
    )
...
def test_dml_blp_omega(dml_blp_fixture):
    assert np.allclose(
        dml_blp_fixture["omega"], dml_blp_fixture["omega_manual"], rtol=1e-9, atol=1e-4
    )
...
def test_dml_blp_ci_1(dml_blp_fixture):
    assert np.allclose(
        dml_blp_fixture["ci_1"], dml_blp_fixture["ci_2"], rtol=1e-9, atol=1e-4
    )
...
def test_dml_blp_ci_2(dml_blp_fixture):
    assert np.allclose(
        dml_blp_fixture["ci_1"], dml_blp_fixture["ci_manual"], rtol=1e-9, atol=1e-4
    )
...
def test_doubleml_exception_blp():
    ...
    with pytest.raises(ValueError, match=msg):
        dml.DoubleMLBLP(
            orth_signal=signal,
            basis=pd.DataFrame(np.array([[1, 2], [4, 5]]), columns=["a_1", "a_1"]),
        )
    ...
    with pytest.raises(ValueError, match=msg):
        dml_blp_confint.confint(
            basis=pd.DataFrame(np.array([[1], [4]]), columns=["a_1"])
        )
    msg = "Invalid basis: DataFrame has to have the exact same number and ordering of columns."
    with pytest.raises(ValueError, match=msg):
        dml_blp_confint.confint(
            basis=pd.DataFrame(
                np.array([[1, 2, 3], [4, 5, 6]]), columns=["x_1", "x_2", "x_3"]
            )
        )
