...
@pytest.fixture(
    scope="module", params=[RandomForestRegressor(max_depth=2, n_estimators=10)]
)
def learner(request):
...
def dml_pliv_no_cross_fit_fixture(generate_data_iv, learner, score, n_folds):
    ...
    for bootstrap in boot_methods:
        np.random.seed(3141)
        boot_theta, boot_t_stat = boot_pliv(
            y,
            d,
            z,
            res_manual["thetas"],
...
def test_dml_pliv_no_cross_fit_coef(dml_pliv_no_cross_fit_fixture):
    assert math.isclose(
        dml_pliv_no_cross_fit_fixture["coef"],
...
def test_dml_pliv_no_cross_fit_se(dml_pliv_no_cross_fit_fixture):
    assert math.isclose(
        dml_pliv_no_cross_fit_fixture["se"],
...
def test_dml_pliv_no_cross_fit_boot(dml_pliv_no_cross_fit_fixture):
    for bootstrap in dml_pliv_no_cross_fit_fixture["boot_methods"]:
        assert np.allclose(
            dml_pliv_no_cross_fit_fixture["boot_coef" + bootstrap],
            dml_pliv_no_cross_fit_fixture["boot_coef" + bootstrap + "_manual"],
            rtol=1e-9,
            atol=1e-4,
        )
        assert np.allclose(
            dml_pliv_no_cross_fit_fixture["boot_t_stat" + bootstrap],
