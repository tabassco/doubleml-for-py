...
def test_doubleml_exception_data():
    ...
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLPLIV(
            DummyDataClass(pd.DataFrame(np.zeros((100, 10)))), ml_l, ml_m, ml_r
        )
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLIRM(DummyDataClass(pd.DataFrame(np.zeros((100, 10)))), ml_g, ml_m)
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLIIVM(
            DummyDataClass(pd.DataFrame(np.zeros((100, 10)))), ml_g, ml_m, ml_r
        )
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLPQ(
            DummyDataClass(pd.DataFrame(np.zeros((100, 10)))), ml_g, ml_m, treatment=1
        )
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLLPQ(
            DummyDataClass(pd.DataFrame(np.zeros((100, 10)))), ml_g, ml_m, treatment=1
    ...
    with pytest.raises(ValueError, match=msg):
        # multiple D for IRM
        _ = DoubleMLIRM(
            DoubleMLData(df_irm, "y", ["d", "X1"]), Lasso(), LogisticRegression()
        )
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = DoubleMLIIVM(
            DoubleMLData(df_iivm, "y", "d", z_cols="z"),
            Lasso(),
            LogisticRegression(),
            LogisticRegression(),
        )
    ...
    with pytest.raises(ValueError, match=msg):
        # multiple D for IIVM
        _ = DoubleMLIIVM(
            DoubleMLData(df_iivm, "y", ["d", "X1"], z_cols="z"),
            Lasso(),
            LogisticRegression(),
            LogisticRegression(),
        )
    ...
    with pytest.raises(ValueError, match=msg):
        # IIVM without IV
        _ = DoubleMLIIVM(
            dml_data_irm, Lasso(), LogisticRegression(), LogisticRegression()
        )
    df_iivm = dml_data_iivm.data.copy()
    ...
    with pytest.raises(ValueError, match=msg):
        # multiple Z for IIVM
        _ = DoubleMLIIVM(
            DoubleMLData(df_iivm, "y", "d", z_cols=["z", "X1"]),
            Lasso(),
            LogisticRegression(),
            LogisticRegression(),
        )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLPQ(
            dml_data_iivm, LogisticRegression(), LogisticRegression(), treatment=1
        )
    msg = (
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = DoubleMLLPQ(
            DoubleMLData(df_iivm, "y", "d", x_cols=["z"]),
            LogisticRegression(),
            LogisticRegression(),
            treatment=1,
        )
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = DoubleMLCVAR(
            DoubleMLData(df_irm, "y", "d"), Lasso(), LogisticRegression(), treatment=1
        )
    df_irm = dml_data_irm.data.copy()
    with pytest.raises(ValueError, match=msg):
        # multiple D for CVAR
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = DoubleMLQTE(
            DoubleMLData(df_irm, "y", "d"), LogisticRegression(), LogisticRegression()
        )
    df_irm = dml_data_irm.data.copy()
    with pytest.raises(ValueError, match=msg):
        # multiple D for QTE
    ...
    with pytest.raises(ValueError, match=msg):
        ...
        _ = DoubleMLDIDCS(
            DoubleMLData(df_did_cs, y_col="y", d_cols="d", t_col="t"),
            Lasso(),
            LogisticRegression(),
        )
    df_did_cs = dml_data_did_cs.data.copy()
    with pytest.raises(ValueError, match=msg):
        ...
        _ = DoubleMLDIDCS(
            DoubleMLData(df_did_cs, y_col="y", d_cols=["d", "Z1"], t_col="t"),
            Lasso(),
            LogisticRegression(),
        )
...
def test_doubleml_exception_scores():
    ...
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLIIVM(
            dml_data_iivm, Lasso(), LogisticRegression(), LogisticRegression(), score=0
        )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLQTE(
            dml_data_irm, LogisticRegression(), LogisticRegression(), score="IV"
        )
    msg = "score should be a string. 2 was passed."
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLQTE(
            dml_data_irm, LogisticRegression(), LogisticRegression(), score=2
        )
...
def test_doubleml_exception_trimming_rule():
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLIRM(
            dml_data_irm, Lasso(), LogisticRegression(), trimming_rule="discard"
        )
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLIIVM(
            dml_data_iivm,
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLDID(
            dml_data_did, Lasso(), LogisticRegression(), trimming_rule="discard"
        )
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLDIDCS(
            dml_data_did_cs, Lasso(), LogisticRegression(), trimming_rule="discard"
        )
...
def test_doubleml_exception_treatment():
    msg = (
        "Treatment indicator has to be an integer. Object of type <class 'str'> passed."
    )
    with pytest.raises(TypeError, match=msg):
...
def test_doubleml_exception_ipw_normalization():
    ...
    with pytest.raises(TypeError, match=msg):
        _ = DoubleMLCVAR(
            dml_data_irm, Lasso(), LogisticRegression(), treatment=1, normalize_ipw=1
        )
...
def test_doubleml_exception_smpls():
    ...
    dml_pliv_cluster_no_smpls = DoubleMLPLIV(
        dml_cluster_data_pliv, ml_l, ml_m, ml_r, draw_sample_splitting=False
    )
    with pytest.raises(ValueError, match=msg):
...
def test_doubleml_exception_bootstrap():
    ...
    dml_qte_boot = DoubleMLQTE(
        dml_data_irm, RandomForestClassifier(), RandomForestClassifier()
    )
...
def test_doubleml_exception_tune():
    ...
    msg = (
        r"Invalid param_grids {'ml_r': {'alpha': \[0.05, 0.5\]}}. "
    ...
    with pytest.raises(ValueError, match=msg):
        dml_plr_iv_type.tune(
            {"ml_g": {"alpha": [0.05, 0.5]}, "ml_m": {"alpha": [0.05, 0.5]}}
        )
...
def test_doubleml_exception_learner():
    ...
    with pytest.warns(UserWarning, match=msg):
        _ = DoubleMLPLR(
            dml_data, ml_l=Lasso(), ml_m=Lasso(), ml_g=Lasso(), score="partialling out"
        )

    msg = "For score = 'IV-type', learners ml_l, ml_m, ml_r and ml_g need to be specified."
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLPLIV(
            dml_data_pliv, ml_l=ml_l, ml_m=ml_m, ml_r=ml_r, score="IV-type"
        )
    ...
    with pytest.warns(UserWarning, match=msg):
        dml_irm_hidden_classifier = DoubleMLIRM(
            dml_data_irm_binary_outcome, log_reg, LogisticRegression()
        )
    msg = (
        r"For the binary outcome variable y, predictions obtained with the ml_g learner "
    ...
    with pytest.raises(ValueError, match=msg):
        dml_irm_hidden_classifier.set_ml_nuisance_params(
            "ml_g0", "d", {"max_iter": 314}
        )
        dml_irm_hidden_classifier.fit()
    ...
    with pytest.raises(ValueError, match=msg):
        dml_iivm_hidden_classifier.set_ml_nuisance_params(
            "ml_g0", "d", {"max_iter": 314}
        )
        dml_iivm_hidden_classifier.fit()
...
def test_doubleml_exception_and_warning_learner():
    ...
    msg = (
        "Invalid learner provided for ml_m: "
        + r"Lasso\(\) has no method .predict_proba\(\)."
    )
    with pytest.raises(TypeError, match=msg):
...
def test_doubleml_sensitivity_not_yet_implemented():
    dml_pliv = DoubleMLPLIV(
        dml_data_pliv, ml_g, ml_m, ml_r, n_folds=1, apply_cross_fitting=False
    )
    dml_pliv.fit()
...
def test_doubleml_sensitivity_inputs():
    dml_irm = DoubleMLIRM(
        dml_data_irm, Lasso(), LogisticRegression(), trimming_threshold=0.1
    )
    dml_irm.fit()
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm._calc_robustness_value(
            rho=1, null_hypothesis=0.0, level=0.95, idx_treatment=0
        )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml_irm._calc_robustness_value(
            rho=1.1, null_hypothesis=0.0, level=0.95, idx_treatment=0
        )
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm._calc_robustness_value(
            rho=1.0, level=1, null_hypothesis=0.0, idx_treatment=0
        )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml_irm._calc_robustness_value(
            rho=1.0, level=1.0, null_hypothesis=0.0, idx_treatment=0
        )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml_irm._calc_robustness_value(
            rho=1.0, level=0.0, null_hypothesis=0.0, idx_treatment=0
        )
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm._calc_robustness_value(
            null_hypothesis=1, level=0.95, rho=1.0, idx_treatment=0
        )
    msg = r"null_hypothesis must be of float type. \[1\] of type <class 'numpy.ndarray'> was passed."
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm._calc_robustness_value(
            null_hypothesis=np.array([1]), level=0.95, rho=1.0, idx_treatment=0
        )
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm._calc_robustness_value(
            idx_treatment=0.0, null_hypothesis=0.0, level=0.95, rho=1.0
        )
    with pytest.raises(TypeError, match=msg):
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml_irm._calc_robustness_value(
            idx_treatment=-1, null_hypothesis=0.0, level=0.95, rho=1.0
        )
    with pytest.raises(ValueError, match=msg):
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm._set_sensitivity_elements(
            sensitivity_elements=1, i_rep=0, i_treat=0
        )

    sensitivity_elements = dict({"sigma2": 1})
    with pytest.raises(ValueError):
        _ = dml_irm._set_sensitivity_elements(
            sensitivity_elements=sensitivity_elements, i_rep=0, i_treat=0
        )

    # test variances
    sensitivity_elements = dict(
        {"sigma2": 1.0, "nu2": -2.4, "psi_sigma2": 1.0, "psi_nu2": 1.0}
    )
    _ = dml_irm._set_sensitivity_elements(
        sensitivity_elements=sensitivity_elements, i_rep=0, i_treat=0
    )
...
def test_doubleml_sensitivity_plot_input():
    ...
    msg = (
        r"Apply sensitivity_analysis\(\) to include senario in sensitivity_plot. "
    ...
    with pytest.raises(ValueError, match=msg):
        _ = dml_irm.sensitivity_plot(
            benchmarks={"cf_y": [0.1], "cf_d": [0.15, 0.2], "name": ["test", "test2"]}
        )
    ...
    with pytest.raises(TypeError, match=msg):
        _ = dml_irm.sensitivity_plot(
            benchmarks={"cf_y": [0.1, 0.2], "cf_d": [0.15, 0.2], "name": [2, 2]}
        )
...
def test_doubleml_cluster_not_yet_implemented():
    ...
    msg = (
        r"No cross-fitting \(`apply_cross_fitting = False`\) "
        "is not yet implemented with clustering."
    )
    with pytest.raises(NotImplementedError, match=msg):
        _ = DoubleMLPLIV(dml_cluster_data_pliv, ml_g, ml_m, ml_r, n_folds=1)
    with pytest.raises(NotImplementedError, match=msg):
        _ = DoubleMLPLIV(
            dml_cluster_data_pliv,
            ml_g,
            ml_m,
            ml_r,
...
def test_double_ml_exception_evaluate_learner():
    ...
    msg = (
        r"The learners have to be a subset of \['ml_g0', 'ml_g1', 'ml_m'\]. "
...
def test_doubleml_exception_policytree():
    ...
    with pytest.raises(ValueError, match=msg):
        dml_irm_obj.policy_tree(
            features=pd.DataFrame(np.random.normal(0, 1, size=(dml_data_irm.n_obs, 3))),
            depth=-1,
        )
