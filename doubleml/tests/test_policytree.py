...
def test_dml_policytree_treshold(dml_policytree_fixture):
    assert np.allclose(
        dml_policytree_fixture["tree"].threshold,
...
def test_dml_policytree_children(dml_policytree_fixture):
    assert np.allclose(
        dml_policytree_fixture["tree"].children_left,
...
def test_dml_policytree_return_types(dml_policytree_fixture):
    ...
    assert isinstance(
        dml_policytree_fixture["policytree_model"].policy_tree, DecisionTreeClassifier
    )
...
def test_doubleml_exception_policytree():
    random_features = pd.DataFrame(
        np.random.normal(0, 1, size=(2, 3)), columns=["a", "b", "c"]
    )
    signal = np.array([1, 2])
    ...
    with pytest.raises(ValueError, match=msg):
        dml.DoubleMLPolicyTree(
            orth_signal=np.array([[1], [2]]), features=random_features
        )
    ...
    with pytest.raises(ValueError, match=msg):
        dml.DoubleMLPolicyTree(
            orth_signal=signal,
            features=pd.DataFrame(np.array([[1, 2], [4, 5]]), columns=["a_1", "a_1"]),
        )

    dml_policytree_predict = dml.DoubleMLPolicyTree(
        orth_signal=signal, features=random_features
    )
    ...
    dml_policytree_plot = dml.DoubleMLPolicyTree(
        orth_signal=signal, features=random_features
    )
