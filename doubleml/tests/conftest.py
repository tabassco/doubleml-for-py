...
def generate_data_toeplitz(request, betamax=4, decay=0.99, threshold=0, noisevar=10):
    ...
    column_names = (
        [f"X{i + 1}" for i in np.arange(x.shape[1])]
        + ["y"]
        + [f"d{i + 1}" for i in np.arange(len(cols_treatment))]
    )
    data = pd.DataFrame(np.column_stack((x, y, d)), columns=column_names)
...
def generate_data_iv(request):
    ...
    data = make_pliv_CHS2015(
        n_obs=n, dim_x=p, alpha=theta, dim_z=1, return_type=pd.DataFrame
    )
...
@pytest.fixture(
    scope="session",
    params=[(253, 10, False), (501, 52, False), (253, 10, True), (501, 52, True)],
)
...
def generate_data_did_cs(request):
    ...
    data = make_did_SZ2020(
        n, dgp_type=dpg, cross_sectional_data=True, return_type="array"
    )
