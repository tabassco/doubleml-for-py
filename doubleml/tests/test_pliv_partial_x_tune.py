...
from ._utils_pliv_partial_x_manual import (
    fit_pliv_partial_x,
    boot_pliv_partial_x,
    tune_nuisance_pliv_partial_x,
)
...
def dml_pliv_partial_x_fixture(
    ...
    np.random.seed(3141)
    dml_pliv_obj = dml.DoubleMLPLIV._partialX(
    ...
    for bootstrap in boot_methods:
        np.random.seed(3141)
        boot_theta, boot_t_stat = boot_pliv_partial_x(
            y,
            d,
            z,
            res_manual["thetas"],
...
def test_dml_pliv_coef(dml_pliv_partial_x_fixture):
    assert math.isclose(
        dml_pliv_partial_x_fixture["coef"],
...
def test_dml_pliv_boot(dml_pliv_partial_x_fixture):
    for bootstrap in dml_pliv_partial_x_fixture["boot_methods"]:
        assert np.allclose(
            dml_pliv_partial_x_fixture["boot_coef" + bootstrap],
