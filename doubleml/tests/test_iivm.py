...
def dml_iivm_fixture(
    generate_data_iivm, learner, score, dml_procedure, normalize_ipw, trimming_threshold
):
    boot_methods = ["normal"]
    n_folds = 2
    n_rep_boot = 491
...
def test_dml_iivm_boot(dml_iivm_fixture):
    for bootstrap in dml_iivm_fixture["boot_methods"]:
        assert np.allclose(
            dml_iivm_fixture["boot_coef" + bootstrap],
