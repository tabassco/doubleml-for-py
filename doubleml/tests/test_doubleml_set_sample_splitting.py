...
ml_m = Lasso()
dml_plr = DoubleMLPLR(
    dml_data, ml_l, ml_m, n_folds=7, n_rep=8, draw_sample_splitting=False
)
        ...
        if apply_cross_fitting:
            for i_fold, _ in enumerate(smpls0[i_rep]):
                assert np.array_equal(
                    smpls0[i_rep][i_fold][0], smpls1[i_rep][i_fold][0]
                )
                assert np.array_equal(
                    smpls0[i_rep][i_fold][1], smpls1[i_rep][i_fold][1]
                )
        else:
...
def test_doubleml_set_sample_splitting_tuple():
    ...
    _assert_smpls_equal(
        [[smpls]], dml_plr.smpls, apply_cross_fitting=dml_plr.apply_cross_fitting
    )
...
def test_doubleml_draw_vs_set():
    ...
    dml_plr_drawn = DoubleMLPLR(
        dml_data, ml_l, ml_m, n_folds=1, n_rep=1, apply_cross_fitting=False
    )
    ...
    dml_plr_drawn = DoubleMLPLR(
        dml_data, ml_l, ml_m, n_folds=2, n_rep=1, apply_cross_fitting=False
    )
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls)
    _assert_resampling_pars(dml_plr_drawn, dml_plr_set)
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls[0])
    ...
    dml_plr_drawn = DoubleMLPLR(
        dml_data, ml_l, ml_m, n_folds=2, n_rep=1, apply_cross_fitting=True
    )
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls)
    _assert_resampling_pars(dml_plr_drawn, dml_plr_set)
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls[0])
    ...
    dml_plr_drawn = DoubleMLPLR(
        dml_data, ml_l, ml_m, n_folds=5, n_rep=1, apply_cross_fitting=True
    )
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls)
    _assert_resampling_pars(dml_plr_drawn, dml_plr_set)
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls[0])
    ...
    dml_plr_drawn = DoubleMLPLR(
        dml_data, ml_l, ml_m, n_folds=5, n_rep=3, apply_cross_fitting=True
    )
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls)
    _assert_resampling_pars(dml_plr_drawn, dml_plr_set)

    dml_plr_drawn = DoubleMLPLR(
        dml_data, ml_l, ml_m, n_folds=2, n_rep=4, apply_cross_fitting=False
    )
    dml_plr_set.set_sample_splitting(dml_plr_drawn.smpls)
    _assert_resampling_pars(dml_plr_drawn, dml_plr_set)
