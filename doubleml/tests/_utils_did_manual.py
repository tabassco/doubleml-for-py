...
def fit_did(
    ...
    for i_rep in range(n_rep):
        ...
        psi_a, psi_b = did_score_elements(
            g_hat0, g_hat1, m_hat, p_hat, resid_d0, d, score, in_sample_normalization
        )
...
def did_score_elements(
    ...
    if score == "observational":
        if in_sample_normalization:
            ...
            weight_resid_d0 = np.divide(d, np.mean(d)) - np.divide(
                propensity_weight, np.mean(propensity_weight)
            )
        else:
...
def var_did(theta, psi_a, psi_b, n_obs):
    ...
    var = (
        1
        / n_obs
        * np.mean(np.power(np.multiply(psi_a, theta) + psi_b, 2))
        / np.power(J, 2)
    )
    return var
...
def boot_did_single_split(
    theta, psi_a, psi_b, smpls, se, weights, n_rep_boot, apply_cross_fitting
):
    if apply_cross_fitting:
        J = np.mean(psi_a)
    else:
...
def tune_nuisance_did(
    y, x, d, ml_g, ml_m, smpls, score, n_folds_tune, param_grid_g, param_grid_m
):
    train_cond0 = np.where(d == 0)[0]
...
def fit_sensitivity_elements_did(
    y, d, all_coef, predictions, score, in_sample_normalization, n_rep
):
    n_treat = 1
    n_obs = len(y)
    ...
    for i_rep in range(n_rep):
        ...
        sigma2_score_element = np.square(
            y - np.multiply(d, g_hat1) - np.multiply(1.0 - d, g_hat0)
        )
        ...
        if score == "observational":
            ...
            if in_sample_normalization:
                ...
                m_alpha_2 = np.divide(1, np.mean(d)) + np.divide(
                    ...
                    np.mean(np.multiply(1.0 - d, propensity_weight_d0)),
                )
                ...
                rr = np.divide(d, np.mean(d)) - np.multiply(
                    np.divide(1.0 - d, np.mean(d)), propensity_weight_d0
                )
        else:
            assert score == "experimental"
