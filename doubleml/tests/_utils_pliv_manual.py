...
def fit_pliv(
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            thetas[i_rep], ses[i_rep] = pliv_dml1(
                y, x, d, z, l_hat, m_hat, r_hat, g_hat, smpls, score
            )
        else:
...
def pliv_dml1(y, x, d, z, l_hat, m_hat, r_hat, g_hat, smpls, score):
    ...
    for idx, (_, test_index) in enumerate(smpls):
        thetas[idx] = pliv_orth(
            y_minus_l_hat[test_index],
            z_minus_m_hat[test_index],
            d_minus_r_hat[test_index],
...
def pliv_dml2(y, x, d, z, l_hat, m_hat, r_hat, g_hat, smpls, score):
    ...
    theta_hat = pliv_orth(
        y_minus_l_hat, z_minus_m_hat, d_minus_r_hat, y_minus_g_hat, d, score
    )
    se = np.sqrt(
        var_pliv(
            theta_hat,
...
def var_pliv(
    theta, d, y_minus_l_hat, z_minus_m_hat, d_minus_r_hat, y_minus_g_hat, score, n_obs
):
    if score == "partialling out":
        var = (
            ...
            * np.mean(
                np.power(
                    np.multiply(y_minus_l_hat - d_minus_r_hat * theta, z_minus_m_hat), 2
                )
            )
        )
    else:
        ...
        var = (
            ...
            * np.mean(
                np.power(np.multiply(y_minus_g_hat - d * theta, z_minus_m_hat), 2)
            )
        )
...
def pliv_orth(y_minus_l_hat, z_minus_m_hat, d_minus_r_hat, y_minus_g_hat, d, score):
    if score == "partialling out":
        res = np.mean(np.multiply(z_minus_m_hat, y_minus_l_hat)) / np.mean(
            np.multiply(z_minus_m_hat, d_minus_r_hat)
        )
    else:
        assert score == "IV-type"
        res = np.mean(np.multiply(z_minus_m_hat, y_minus_g_hat)) / np.mean(
            np.multiply(z_minus_m_hat, d)
        )
...
def boot_pliv_single_split(
    ...
    if apply_cross_fitting:
        ...
        if score == "partialling out":
            J = np.mean(
                -np.multiply(z_minus_m_hat[test_index], d_minus_r_hat[test_index])
            )
        else:
            assert score == "IV-type"
