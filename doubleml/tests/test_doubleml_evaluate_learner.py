...
def dml_irm_eval_learner_fixture(learner, dml_procedure, trimming_threshold, n_rep):
    ...
    res_dict = {"rmses": dml_irm_obj.rmses, "rmses_manual": res_manual}
    return res_dict
...
def test_dml_irm_eval_learner(dml_irm_eval_learner_fixture, n_rep):
    ...
    assert np.allclose(
        dml_irm_eval_learner_fixture["rmses_manual"]["ml_g0"],
        dml_irm_eval_learner_fixture["rmses"]["ml_g0"],
        rtol=1e-9,
        atol=1e-4,
    )
    assert np.allclose(
        dml_irm_eval_learner_fixture["rmses_manual"]["ml_g1"],
        dml_irm_eval_learner_fixture["rmses"]["ml_g1"],
        rtol=1e-9,
        atol=1e-4,
    )
    assert np.allclose(
        dml_irm_eval_learner_fixture["rmses_manual"]["ml_m"],
