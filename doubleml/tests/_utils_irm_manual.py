...
def fit_irm(
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            thetas[i_rep], ses[i_rep] = irm_dml1(
                y, x, d, g_hat0, g_hat1, m_hat, p_hat, smpls, score, normalize_ipw
            )
        else:
            assert dml_procedure == "dml2"
            thetas[i_rep], ses[i_rep] = irm_dml2(
                y, x, d, g_hat0, g_hat1, m_hat, p_hat, smpls, score, normalize_ipw
            )
...
def irm_dml1(
    ...
    if len(smpls) > 1:
        se = np.sqrt(
            var_irm(
                theta_hat, g_hat0, g_hat1, m_hat, p_hat, u_hat0, u_hat1, d, score, n_obs
            )
        )
    else:
        assert len(smpls) == 1
...
def irm_dml2(
    ...
    theta_hat = irm_orth(g_hat0, g_hat1, m_hat, p_hat, u_hat0, u_hat1, d, score)
    se = np.sqrt(
        var_irm(
            theta_hat, g_hat0, g_hat1, m_hat, p_hat, u_hat0, u_hat1, d, score, n_obs
        )
        ...
        var = (
            ...
            * np.mean(
                np.power(
                    ...
                    - np.divide(
                        np.multiply(m_hat, np.multiply(1.0 - d, u_hat0)),
...
def fit_sensitivity_elements_irm(y, d, all_coef, predictions, score, n_rep):
    ...
    for i_rep in range(n_rep):
        ...
        m_alpha = np.multiply(
            weights,
            np.multiply(
                weights_bar, (np.divide(1.0, m_hat) + np.divide(1.0, 1.0 - m_hat))
            ),
        )
        rr = np.multiply(
            weights_bar, (np.divide(d, m_hat) - np.divide(1.0 - d, 1.0 - m_hat))
        )
