...
dml_pliv = DoubleMLPLIV(
    dml_data_pliv, reg_learner, reg_learner, reg_learner, n_folds=n_folds
)
dml_irm = DoubleMLIRM(dml_data_irm, reg_learner, class_learner, n_folds=n_folds)
dml_iivm = DoubleMLIIVM(
    dml_data_iivm, reg_learner, class_learner, class_learner, n_folds=n_folds
)
dml_cvar = DoubleMLCVAR(
    dml_data_irm, ml_g=reg_learner, ml_m=class_learner, n_folds=n_folds
)
...
# nonlinear models
dml_pq = DoubleMLPQ(
    dml_data_irm, ml_g=class_learner, ml_m=class_learner, n_folds=n_folds
)
dml_lpq = DoubleMLLPQ(
    dml_data_iivm, ml_g=class_learner, ml_m=class_learner, n_folds=n_folds
)
...
def _assert_nuisance_params(dml_obj, learner_1, learner_2):
    ...
    param_list_1 = [
        dml_obj.models[learner_1]["d"][0][fold].n_estimators for fold in range(n_folds)
    ]
    assert all(param == n_est_test for param in param_list_1)
    param_list_2 = [
        dml_obj.models[learner_2]["d"][0][fold].n_estimators for fold in range(n_folds)
    ]
...
def test_lpq_params():
    ...
    param_list_2 = [
        dml_lpq.models["ml_m_d_z1"]["d"][0][fold].n_estimators
        for fold in range(n_folds)
    ]
    assert all(param == n_est_default for param in param_list_2)
    param_list_2 = [
        dml_lpq.models["ml_g_du_z0"]["d"][0][fold].n_estimators
        for fold in range(n_folds)
    ]
    assert all(param == n_est_default for param in param_list_2)
    param_list_2 = [
        dml_lpq.models["ml_g_du_z1"]["d"][0][fold].n_estimators
        for fold in range(n_folds)
    ]
    assert all(param == n_est_default for param in param_list_2)
