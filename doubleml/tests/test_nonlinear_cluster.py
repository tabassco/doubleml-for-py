...
@pytest.mark.ci
def test_dml_plr_oneway_cluster_linear_vs_nonlinear_coef(
    dml_plr_oneway_cluster_linear_vs_nonlinear_fixture,
):
    assert math.isclose(
        dml_plr_oneway_cluster_linear_vs_nonlinear_fixture["coef_linear"],
...
@pytest.mark.ci
def test_dml_plr_oneway_cluster_linear_vs_nonlinear_se(
    dml_plr_oneway_cluster_linear_vs_nonlinear_fixture,
):
    assert math.isclose(
        dml_plr_oneway_cluster_linear_vs_nonlinear_fixture["se_linear"],
...
@pytest.mark.ci
def test_dml_plr_multiway_cluster_linear_vs_nonlinear_coef(
    dml_plr_multiway_cluster_linear_vs_nonlinear_fixture,
):
    assert math.isclose(
        dml_plr_multiway_cluster_linear_vs_nonlinear_fixture["coef_linear"],
...
@pytest.mark.ci
def test_dml_plr_multiway_cluster_linear_vs_nonlinear_se(
    dml_plr_multiway_cluster_linear_vs_nonlinear_fixture,
):
    assert math.isclose(
        dml_plr_multiway_cluster_linear_vs_nonlinear_fixture["se_linear"],
...
def dml_plr_cluster_nonlinear_with_index(generate_data1, learner, dml_procedure):
    ...
    np.random.seed(3141)
    dml_plr_obj = DoubleMLPLRWithNonLinearScoreMixin(
        obj_dml_data, ml_l, ml_m, n_folds=n_folds, dml_procedure=dml_procedure
    )
    dml_plr_obj.fit()

    df = data.reset_index()
    dml_cluster_data = dml.DoubleMLClusterData(
        df, y_col="y", d_cols="d", x_cols=x_cols, cluster_cols="index"
    )
    np.random.seed(3141)
    dml_plr_cluster_obj = DoubleMLPLRWithNonLinearScoreMixin(
        dml_cluster_data, ml_l, ml_m, n_folds=n_folds, dml_procedure=dml_procedure
    )
...
@pytest.mark.ci
def test_dml_plr_cluster_nonlinear_with_index_coef(
    dml_plr_cluster_nonlinear_with_index,
):
    assert math.isclose(
        dml_plr_cluster_nonlinear_with_index["coef"],
...
def test_dml_plr_cluster_nonlinear_with_index_se(dml_plr_cluster_nonlinear_with_index):
    assert math.isclose(
        dml_plr_cluster_nonlinear_with_index["se"],
