...
def fit_pliv_partial_xz(
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            thetas[i_rep], ses[i_rep] = pliv_partial_xz_dml1(
                y, x, d, z, l_hat, m_hat, r_hat, smpls, score
            )
        else:
            assert dml_procedure == "dml2"
            thetas[i_rep], ses[i_rep] = pliv_partial_xz_dml2(
                y, x, d, z, l_hat, m_hat, r_hat, smpls, score
            )
...
def fit_nuisance_pliv_partial_xz(
    ...
    for idx, (train_index, test_index) in enumerate(smpls):
        ...
        m_hat_tilde.append(
            ml_r.fit(x[train_index], m_hat_train[idx]).predict(x[test_index])
        )

    return l_hat, m_hat, m_hat_tilde


def tune_nuisance_pliv_partial_xz(
    y,
    x,
    d,
    z,
...
def pliv_partial_xz_dml1(y, x, d, z, l_hat, m_hat, m_hat_tilde, smpls, score):
    ...
    n_obs = len(y)
    u_hat, v_hat, w_hat = compute_pliv_partial_xz_residuals(
        y, d, l_hat, m_hat, m_hat_tilde, smpls
    )

    for idx, (_, test_index) in enumerate(smpls):
        thetas[idx] = pliv_partial_xz_orth(
            u_hat[test_index],
            v_hat[test_index],
            w_hat[test_index],
            d[test_index],
...
def boot_pliv_partial_xz_single_split(
    theta, y, d, z, l_hat, m_hat, m_hat_tilde, smpls, score, se, weights, n_rep_boot
):
    assert score == "partialling out"
    u_hat, v_hat, w_hat = compute_pliv_partial_xz_residuals(
        y, d, l_hat, m_hat, m_hat_tilde, smpls
