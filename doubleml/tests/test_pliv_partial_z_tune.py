...
from ._utils_pliv_partial_z_manual import (
    fit_pliv_partial_z,
    boot_pliv_partial_z,
    tune_nuisance_pliv_partial_z,
)
...
@pytest.fixture(scope="module", params=["dml2"])
...
def dml_pliv_partial_z_fixture(
    generate_data_pliv_partialZ, learner_r, score, dml_procedure, tune_on_folds
):
    ...
    dml_pliv_obj = dml.DoubleMLPLIV._partialZ(
        obj_dml_data, ml_r, n_folds=n_folds, dml_procedure=dml_procedure
    )
    ...
    if tune_on_folds:
        r_params = tune_nuisance_pliv_partial_z(
            y, x, d, z, clone(learner_r), smpls, n_folds_tune, par_grid["ml_r"]
        )
    else:
        xx = [(np.arange(len(y)), np.array([]))]
        r_params = tune_nuisance_pliv_partial_z(
            y, x, d, z, clone(learner_r), xx, n_folds_tune, par_grid["ml_r"]
        )
        r_params = r_params * n_folds

    res_manual = fit_pliv_partial_z(
        y, x, d, z, clone(learner_r), all_smpls, dml_procedure, score, r_params=r_params
    )
    ...
    for bootstrap in boot_methods:
        np.random.seed(3141)
        boot_theta, boot_t_stat = boot_pliv_partial_z(
            y,
            d,
            z,
            res_manual["thetas"],
...
def test_dml_pliv_coef(dml_pliv_partial_z_fixture):
    assert math.isclose(
        dml_pliv_partial_z_fixture["coef"],
...
def test_dml_pliv_boot(dml_pliv_partial_z_fixture):
    for bootstrap in dml_pliv_partial_z_fixture["boot_methods"]:
        assert np.allclose(
            dml_pliv_partial_z_fixture["boot_coef" + bootstrap],
