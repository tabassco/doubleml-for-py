...
class DoubleMLMultiwayResampling:
    def __init__(self, n_folds, smpl_sizes):
        ...
        assert len(
            smpl_sizes
        ), "For DoubleMLMultiwayResampling mmultiple sample sizes need to be provided"
    ...
    def split_samples(self):
            ...
            smpls.append(
                [
                    ...
                    for train, test in self.resampling.split(
                        np.zeros(self.smpl_sizes[i_way])
                    )
                ]
            )
        ...
        for ind_index_set in itertools.product(*xx):
            ...
            smpls_multi_ind.append(
                (
                    pd.MultiIndex.from_product(smpls_train_list).values,
                    pd.MultiIndex.from_product(smpls_test_list).values,
                )
            )

        smpls_lin_ind = [
            (multi_to_lin_ind.loc[x[0]].values, multi_to_lin_ind.loc[x[1]].values)
            for x in smpls_multi_ind
        ]
