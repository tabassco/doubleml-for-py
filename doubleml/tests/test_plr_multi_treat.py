...
@pytest.fixture(
    scope="module",
    params=[Lasso(alpha=0.1), RandomForestRegressor(max_depth=2, n_estimators=10)],
)
def learner(request):
    return request.param
...
def dml_plr_multitreat_fixture(
    generate_data_bivariate,
    generate_data_toeplitz,
    idx,
    learner,
    score,
    dml_procedure,
...
def test_dml_plr_multitreat_coef(dml_plr_multitreat_fixture):
    assert np.allclose(
        dml_plr_multitreat_fixture["coef"],
...
def test_dml_plr_multitreat_boot(dml_plr_multitreat_fixture):
    for bootstrap in dml_plr_multitreat_fixture["boot_methods"]:
        assert np.allclose(
            dml_plr_multitreat_fixture["boot_coef" + bootstrap],
...
def test_dml_plr_multitreat_sensitivity(dml_plr_multitreat_fixture):
    ...
    for sensitivity_element in sensitivity_element_names:
        assert np.allclose(
            ...
            dml_plr_multitreat_fixture["sensitivity_elements_manual"][
                sensitivity_element
            ],
        )
