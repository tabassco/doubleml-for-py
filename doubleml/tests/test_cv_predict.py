...
def cv_predict_fixture(generate_data_cv_predict, cross_fit, params):
    ...
    if method == "predict_proba":
        preds = _dml_cv_predict(
            LogisticRegression(), x, y, smpls, est_params=est_params, method=method
        )
        preds_ut = _dml_cv_predict_ut_version(
            LogisticRegression(), x, y, smpls, est_params=est_params, method=method
        )[:, 1]
    else:
        preds = _dml_cv_predict(
            Lasso(), x, y, smpls, est_params=est_params, method=method
        )
        preds_ut = _dml_cv_predict_ut_version(
            Lasso(), x, y, smpls, est_params=est_params, method=method
        )
...
def test_cv_predict(cv_predict_fixture):
    ...
    assert np.allclose(
        cv_predict_fixture["preds"][~ind_nan_preds],
