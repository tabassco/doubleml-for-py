...
dml_plr_callable_score = DoubleMLPLR(
...
dml_plr_non_orth_score_w_g = DoubleMLPLR(
    dml_data_plr, Lasso(), Lasso(), Lasso(), score=non_orth_score_w_g
)
dml_plr_non_orth_score_w_g.fit(store_predictions=True)

dml_plr_non_orth_score_w_l = DoubleMLPLR(
    dml_data_plr, Lasso(), Lasso(), score=non_orth_score_w_l
)
...
@pytest.mark.parametrize(
    "dml_obj", [dml_plr, dml_plr_iv_type, dml_pliv, dml_irm, dml_iivm]
)
...
def test_plr_callable_vs_pred_export():
    ...
    assert np.allclose(
        dml_plr.psi_elements["psi_a"].squeeze(), psi_a, rtol=1e-9, atol=1e-4
    )
    assert np.allclose(
        dml_plr.psi_elements["psi_b"].squeeze(), psi_b, rtol=1e-9, atol=1e-4
    )
...
def test_plr_iv_type_callable_vs_str_score():
    assert np.allclose(
        dml_plr_iv_type.psi, dml_plr_iv_type_callable_score.psi, rtol=1e-9, atol=1e-4
    )
    assert np.allclose(
        dml_plr_iv_type.coef, dml_plr_iv_type_callable_score.coef, rtol=1e-9, atol=1e-4
    )
...
def test_plr_iv_type_callable_vs_pred_export():
    ...
    assert np.allclose(
        dml_plr_iv_type.psi_elements["psi_a"].squeeze(), psi_a, rtol=1e-9, atol=1e-4
    )
    assert np.allclose(
        dml_plr_iv_type.psi_elements["psi_b"].squeeze(), psi_b, rtol=1e-9, atol=1e-4
    )
...
def test_plr_non_orth_score_w_g_callable_vs_pred_export():
    ...
    m_hat = preds["ml_m"].squeeze()
    g_hat = preds["ml_g"].squeeze()
    psi_a, psi_b = non_orth_score_w_g(
        dml_data_plr.y,
        dml_data_plr.d,
        l_hat,
        m_hat,
    ...
    assert np.allclose(
        dml_plr_non_orth_score_w_g.psi_elements["psi_a"].squeeze(),
        psi_a,
        rtol=1e-9,
        atol=1e-4,
    )
    assert np.allclose(
        dml_plr_non_orth_score_w_g.psi_elements["psi_b"].squeeze(),
        psi_b,
        rtol=1e-9,
        atol=1e-4,
    )
...
def test_plr_non_orth_score_w_l_callable_vs_pred_export():
    ...
    m_hat = preds["ml_m"].squeeze()
    g_hat = None
    psi_a, psi_b = non_orth_score_w_l(
        dml_data_plr.y,
        dml_data_plr.d,
        l_hat,
        m_hat,
    ...
    assert np.allclose(
        dml_plr_non_orth_score_w_l.psi_elements["psi_a"].squeeze(),
        psi_a,
        rtol=1e-9,
        atol=1e-4,
    )
    assert np.allclose(
        dml_plr_non_orth_score_w_l.psi_elements["psi_b"].squeeze(),
        psi_b,
        rtol=1e-9,
        atol=1e-4,
    )
...
def test_irm_callable_vs_pred_export():
    ...
    assert np.allclose(
        dml_irm.psi_elements["psi_a"].squeeze(), psi_a, rtol=1e-9, atol=1e-4
    )
    assert np.allclose(
        dml_irm.psi_elements["psi_b"].squeeze(), psi_b, rtol=1e-9, atol=1e-4
    )
...
def test_iivm_callable_vs_str_score():
    ...
    assert np.allclose(
        dml_iivm.coef, dml_iivm_callable_score.coef, rtol=1e-9, atol=1e-4
    )
...
def test_iivm_callable_vs_pred_export():
    ...
    assert np.allclose(
        dml_iivm.psi_elements["psi_a"].squeeze(), psi_a, rtol=1e-9, atol=1e-4
    )
    assert np.allclose(
        dml_iivm.psi_elements["psi_b"].squeeze(), psi_b, rtol=1e-9, atol=1e-4
    )
...
def test_pliv_callable_vs_str_score():
    ...
    assert np.allclose(
        dml_pliv.coef, dml_pliv_callable_score.coef, rtol=1e-9, atol=1e-4
    )
...
def test_pliv_callable_not_implemented():
    ...
    dml_pliv_callable_score = DoubleMLPLIV._partialX(
        dml_data_pliv_2z, Lasso(), Lasso(), Lasso(), score=pliv_score
    )
    msg = "Callable score not implemented for DoubleMLPLIV.partialX with several instruments."
    with pytest.raises(NotImplementedError, match=msg):
        dml_pliv_callable_score.fit()

    dml_pliv_callable_score = DoubleMLPLIV._partialZ(
        dml_data_pliv_2z, Lasso(), score=pliv_score
    )
    msg = "Callable score not implemented for DoubleMLPLIV.partialZ."
    with pytest.raises(NotImplementedError, match=msg):
        dml_pliv_callable_score.fit()

    dml_pliv_callable_score = DoubleMLPLIV._partialXZ(
        dml_data_pliv_2z, Lasso(), Lasso(), Lasso(), score=pliv_score
    )
