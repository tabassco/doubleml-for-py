...
def dml_data_fixture(generate_data1):
    ...
    obj_from_np = DoubleMLData.from_arrays(
        data.loc[:, x_cols].values, data["y"].values, data["d"].values
    )
...
def test_dml_data_x(dml_data_fixture):
    assert np.allclose(
        dml_data_fixture["obj_from_np"].x,
...
def test_dml_data_y(dml_data_fixture):
    assert np.allclose(
        dml_data_fixture["obj_from_np"].y,
...
def test_dml_data_d(dml_data_fixture):
    assert np.allclose(
        dml_data_fixture["obj_from_np"].d,
...
def test_obj_vs_from_arrays():
    ...
    df.rename(
        columns={"cluster_var_j": "cluster_var", "Y": "y", "D": "d", "Z": "z"},
        inplace=True,
    )
    assert dml_data_from_array.data.equals(df)
...
def test_add_vars_in_df():
    ...
    dml_data_subset = DoubleMLData(
        df[["X1", "X11", "X13"] + ["y", "d"]], "y", "d", ["X1", "X11", "X13"]
    )
    dml_plr_full_df = DoubleMLPLR(dml_data_full_df, Lasso(), Lasso())
    dml_plr_subset = DoubleMLPLR(
        dml_data_subset, Lasso(), Lasso(), draw_sample_splitting=False
    )
...
def test_x_cols_setter_defaults():
    ...
    # with instrument
    df = pd.DataFrame(
        np.tile(np.arange(5), (4, 1)), columns=["yy", "dd", "xx1", "xx2", "zz"]
    )
    ...
    df = pd.DataFrame(
        np.tile(np.arange(5), (4, 1)), columns=["yy", "dd", "xx1", "xx2", "tt"]
    )
    ...
    df = pd.DataFrame(
        np.tile(np.arange(6), (4, 1)), columns=["yy", "dd", "xx1", "xx2", "zz", "tt"]
    )
...
def test_x_cols_setter_defaults_w_cluster():
    ...
    dml_data = DoubleMLClusterData(
        df, y_col="yy", d_cols="dd", cluster_cols="cluster1", z_cols="z"
    )
    assert dml_data.x_cols == ["xx1", "xx2"]
    ...
    dml_data = DoubleMLClusterData(
        df, y_col="yy", d_cols="dd", cluster_cols="cluster1", t_col="tt"
    )
    assert dml_data.x_cols == ["xx1", "xx2"]

    # with instrument and with time
    df = pd.DataFrame(
        np.tile(np.arange(7), (6, 1)),
        columns=["yy", "dd", "xx1", "xx2", "zz", "tt", "cluster1"],
    )
    dml_data = DoubleMLClusterData(
        df, y_col="yy", d_cols="dd", cluster_cols="cluster1", z_cols="zz", t_col="tt"
    )
    assert dml_data.x_cols == ["xx1", "xx2"]
...
def test_x_cols_setter():
    ...
    msg = (
        r"The covariates x_cols must be of str or list type \(or None\). "
...
def test_d_cols_setter():
    ...
    msg = (
        r"The treatment variable\(s\) d_cols must be of str or list type. "
        "5 of type <class 'int'> was passed."
    )
    with pytest.raises(TypeError, match=msg):
        dml_data.d_cols = 5
...
def test_t_col_setter():
    ...
    dml_data = DoubleMLData(
        df, "y", "d", [f"Z{i + 1}" for i in np.arange(4)], t_col="t"
    )
    ...
    msg = (
        r"The time variable t_col must be of str type \(or None\). "
        "5 of type <class 'int'> was passed."
    )
    with pytest.raises(TypeError, match=msg):
        dml_data.t_col = 5
...
def test_cluster_cols_setter():
    ...
    msg = (
        r"The cluster variable\(s\) cluster_cols must be of str or list type. "
        "5 of type <class 'int'> was passed."
    )
    with pytest.raises(TypeError, match=msg):
...
def test_y_col_setter():
    ...
    msg = (
        r"The outcome variable y_col must be of str type. "
        "5 of type <class 'int'> was passed."
    )
    with pytest.raises(TypeError, match=msg):
        dml_data.y_col = 5
...
def test_disjoint_sets():
    np.random.seed(3141)
    df = pd.DataFrame(
        np.tile(np.arange(4), (4, 1)), columns=["yy", "dd1", "xx1", "xx2"]
    )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], z_cols="yy"
        )
    msg = (
        r"At least one variable/column is set as treatment variable \(``d_cols``\) and instrumental variable in "
        "``z_cols``."
    )
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], z_cols=["dd1"]
        )
    msg = (
        r"At least one variable/column is set as covariate \(``x_cols``\) and instrumental variable in "
        "``z_cols``."
    )
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], z_cols="xx2"
        )
    msg = "xx2 cannot be set as time variable ``t_col`` and covariate in ``x_cols``."
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], t_col="xx2"
        )
    msg = "dd1 cannot be set as time variable ``t_col`` and treatment variable in ``d_cols``."
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], t_col="dd1"
        )
    msg = "yy cannot be set as time variable ``t_col`` and outcome variable ``y_col``."
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], t_col="yy"
        )
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLClusterData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], cluster_cols="yy"
        )
    msg = (
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLClusterData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], cluster_cols="dd1"
        )
    msg = (
        r"At least one variable/column is set as covariate \(``x_cols``\) and cluster variable in "
        "``cluster_cols``."
    )
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLClusterData(
            df, y_col="yy", d_cols=["dd1"], x_cols=["xx1", "xx2"], cluster_cols="xx2"
        )
    msg = (
...
def test_duplicates():
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            dml_data.data, y_col="y", d_cols=["d", "d", "X1"], x_cols=["X3", "X2"]
        )
    with pytest.raises(ValueError, match=msg):
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            dml_data.data, y_col="y", d_cols=["d"], x_cols=["X3", "X2", "X3"]
        )
    with pytest.raises(ValueError, match=msg):
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLClusterData(
            dml_cluster_data.data,
            y_col="y",
            d_cols=["d"],
            cluster_cols=["X3", "X2", "X3"],
        )
    with pytest.raises(ValueError, match=msg):
...
def test_dml_datatype():
    ...
    with pytest.raises(TypeError):
        _ = DoubleMLClusterData(
            data_array, y_col="y", d_cols=["d"], cluster_cols=["X3", "X2"]
        )
...
def test_dml_data_w_missings(generate_data_irm_w_missings):
    ...
    with pytest.raises(ValueError, match=msg):
        _ = DoubleMLData(
            dml_data.data, y_col="y", d_cols="d", force_all_x_finite="allownan"
        )
