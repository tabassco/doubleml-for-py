...
def boot_manual(psi, J, smpls, se, weights, n_rep, apply_cross_fitting=True):
    ...
    for i_rep in range(n_rep):
        ...
        if apply_cross_fitting:
            boot_theta[i_rep] = np.mean(np.multiply(this_weights, psi / J))
            boot_t_stat[i_rep] = np.mean(
                np.multiply(np.divide(this_weights, se), psi / J)
            )
        else:
            test_index = smpls[0][1]
            boot_theta[i_rep] = np.mean(np.multiply(this_weights, psi[test_index] / J))
            boot_t_stat[i_rep] = np.mean(
                np.multiply(np.divide(this_weights, se), psi[test_index] / J)
            )
