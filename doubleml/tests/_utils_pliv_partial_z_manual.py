...
def fit_pliv_partial_z(
    y, x, d, z, learner_r, all_smpls, dml_procedure, score, n_rep=1, r_params=None
):
    n_obs = len(y)
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            thetas[i_rep], ses[i_rep] = pliv_partial_z_dml1(
                y, x, d, z, r_hat, smpls, score
            )
        else:
            assert dml_procedure == "dml2"
            thetas[i_rep], ses[i_rep] = pliv_partial_z_dml2(
                y, x, d, z, r_hat, smpls, score
            )
...
def pliv_partial_z_dml1(y, x, d, z, r_hat, smpls, score):
    ...
    for idx, (_, test_index) in enumerate(smpls):
        thetas[idx] = pliv_partial_z_orth(
            r_hat_array[test_index], y[test_index], d[test_index], score
        )
...
def var_pliv_partial_z(theta, r_hat, y, d, score, n_obs):
    ...
    var = (
        ...
        / n_obs
        * 1
        / np.power(np.mean(np.multiply(r_hat, d)), 2)
...
def boot_pliv_partial_z(
    y, d, z, thetas, ses, all_r_hat, all_smpls, score, bootstrap, n_rep_boot, n_rep=1
):
    all_boot_theta = list()
...
def boot_pliv_partial_z_single_split(
    theta, y, d, z, r_hat, smpls, score, se, weights, n_rep_boot
):
    assert score == "partialling out"
    r_hat_array = compute_pliv_partial_z_residuals(y, r_hat, smpls)
