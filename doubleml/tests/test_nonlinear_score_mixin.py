...
class DoubleMLPLRWithNonLinearScoreMixin(NonLinearScoreMixin, DoubleML):
    ...
    def _nuisance_est(self, smpls, n_jobs_cv, return_models=False):
        ...
        psi_a, psi_b = self._score_elements(
            y, d, l_hat["preds"], m_hat["preds"], g_hat["preds"], smpls
        )
...
def dml_plr_w_nonlinear_mixin_fixture(
    generate_data1, learner, score, dml_procedure, coef_bounds
):
    n_folds = 3
...
def test_dml_plr_coef(dml_plr_w_nonlinear_mixin_fixture):
    assert math.isclose(
        dml_plr_w_nonlinear_mixin_fixture["coef"],
...
def test_dml_plr_se(dml_plr_w_nonlinear_mixin_fixture):
    assert math.isclose(
        dml_plr_w_nonlinear_mixin_fixture["se"],
...
def test_nonlinear_warnings(generate_data1, coef_bounds):
    ...
    dml_plr_obj = DoubleMLPLRWithNonLinearScoreMixin(
        obj_dml_data, LinearRegression(), LinearRegression(), score="no_root_pos"
    )
    ...
    dml_plr_obj = DoubleMLPLRWithNonLinearScoreMixin(
        obj_dml_data, LinearRegression(), LinearRegression(), score="no_root_neg"
    )
    msg = "Could not find a root of the score function."
    with pytest.warns(UserWarning, match=msg):
        dml_plr_obj._coef_bounds = coef_bounds
