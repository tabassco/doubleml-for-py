...
from ._utils_pliv_partial_xz_manual import (
    fit_pliv_partial_xz,
    boot_pliv_partial_xz,
    tune_nuisance_pliv_partial_xz,
)
...
def dml_pliv_partial_xz_fixture(
    ...
    np.random.seed(3141)
    dml_pliv_obj = dml.DoubleMLPLIV._partialXZ(
        obj_dml_data, ml_l, ml_m, ml_r, n_folds=n_folds, dml_procedure=dml_procedure
    )
    ...
    for bootstrap in boot_methods:
        np.random.seed(3141)
        boot_theta, boot_t_stat = boot_pliv_partial_xz(
            y,
            d,
            z,
            res_manual["thetas"],
...
def test_dml_pliv_coef(dml_pliv_partial_xz_fixture):
    assert math.isclose(
        dml_pliv_partial_xz_fixture["coef"],
...
def test_dml_pliv_se(dml_pliv_partial_xz_fixture):
    assert math.isclose(
        dml_pliv_partial_xz_fixture["se"],
...
def test_dml_pliv_boot(dml_pliv_partial_xz_fixture):
    for bootstrap in dml_pliv_partial_xz_fixture["boot_methods"]:
        assert np.allclose(
            dml_pliv_partial_xz_fixture["boot_coef" + bootstrap],
