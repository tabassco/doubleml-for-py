...
def dml_did_cs_fixture(
    generate_data_did_cs,
    learner,
    score,
    ...
    for bootstrap in boot_methods:
        np.random.seed(3141)
        boot_theta, boot_t_stat = boot_did(
            y,
            res_manual["thetas"],
            res_manual["ses"],
...
def test_dml_did_cs_boot(dml_did_cs_fixture):
    for bootstrap in dml_did_cs_fixture["boot_methods"]:
        assert np.allclose(
            dml_did_cs_fixture["boot_coef" + bootstrap],
...
@pytest.mark.ci
def test_dml_did_cs_experimental(
    generate_data_did_cs, in_sample_normalization, learner
):
    # collect data
    (x, y, d, t) = generate_data_did_cs
