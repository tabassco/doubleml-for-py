...
def test_dml_did_se(dml_did_fixture):
    assert math.isclose(
        dml_did_fixture["se"], dml_did_fixture["se_manual"], rel_tol=1e-9, abs_tol=1e-4
    )
...
def test_dml_did_boot(dml_did_fixture):
    for bootstrap in dml_did_fixture["boot_methods"]:
        assert np.allclose(
            dml_did_fixture["boot_coef" + bootstrap],
