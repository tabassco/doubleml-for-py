...
from ._utils_doubleml_sensitivity_manual import (
    doubleml_sensitivity_manual,
...
def dml_sensitivity_multitreat_fixture(
    generate_data_bivariate, dml_procedure, n_rep, cf_y, cf_d, rho, level
):
    # collect data
    ...
    dml_plr_obj.sensitivity_analysis(
        cf_y=cf_y, cf_d=cf_d, rho=rho, level=level, null_hypothesis=0.0
    )
...
def test_dml_sensitivity_params(dml_sensitivity_multitreat_fixture):
    ...
    for sensitivity_param in sensitivity_param_names:
        for bound in ["lower", "upper"]:
            assert np.allclose(
                dml_sensitivity_multitreat_fixture["sensitivity_params"][
                    sensitivity_param
                ][bound],
                dml_sensitivity_multitreat_fixture["sensitivity_params_manual"][
                    sensitivity_param
                ][bound],
            )
...
def test_dml_sensitivity_benchmark(dml_sensitivity_multitreat_fixture):
    ...
    assert all(
        dml_sensitivity_multitreat_fixture["benchmark"].columns == expected_columns
    )
    assert all(
        dml_sensitivity_multitreat_fixture["benchmark"].index
        == dml_sensitivity_multitreat_fixture["d_cols"]
    )
    assert dml_sensitivity_multitreat_fixture["benchmark"].equals(
        dml_sensitivity_multitreat_fixture["benchmark_manual"]
    )
