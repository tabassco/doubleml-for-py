...
def draw_smpls(n_obs, n_folds, n_rep=1, groups=None):
    ...
    for _ in range(n_rep):
        ...
        smpls = [
            (train, test)
            for train, test in resampling.split(X=np.zeros(n_obs), y=groups)
        ]
...
def fit_predict(y, x, ml_model, params, smpls, train_cond=None):
    ...
    for idx, (train_index, test_index) in enumerate(smpls):
        ...
        if train_cond is None:
            y_hat.append(
                ml_model.fit(x[train_index], y[train_index]).predict(x[test_index])
            )
        else:
            train_index_cond = np.intersect1d(train_cond, train_index)
            y_hat.append(
                ml_model.fit(x[train_index_cond], y[train_index_cond]).predict(
                    x[test_index]
                )
            )
...
def fit_predict_proba(
    y, x, ml_model, params, smpls, trimming_threshold=0, train_cond=None
):
    y_hat = []
    for idx, (train_index, test_index) in enumerate(smpls):
        if params is not None:
            ml_model.set_params(**params[idx])
        if train_cond is None:
            preds = ml_model.fit(x[train_index], y[train_index]).predict_proba(
                x[test_index]
            )[:, 1]
        else:
            train_index_cond = np.intersect1d(train_cond, train_index)
            preds = ml_model.fit(
                x[train_index_cond], y[train_index_cond]
...
def tune_grid_search(y, x, ml_model, smpls, param_grid, n_folds_tune, train_cond=None):
    ...
    for idx, (train_index, _) in enumerate(smpls):
        ...
        if train_cond is None:
            ...
            train_index_cond = np.intersect1d(train_cond, train_index)
            tune_res[idx] = g_grid_search.fit(
                x[train_index_cond, :], y[train_index_cond]
            )
