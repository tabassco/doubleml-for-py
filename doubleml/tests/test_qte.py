...
def dml_qte_fixture(
    generate_data_quantiles, learner, dml_procedure, normalize_ipw, kde
):
    n_folds = 3
    boot_methods = ["normal"]
    n_rep_boot = 2
    ...
    for bootstrap in boot_methods:
        np.random.seed(42)
        boot_qte_coef, boot_t_stat = boot_qte(
            res_manual["scaled_scores"],
            res_manual["ses"],
            quantiles,
            all_smpls,
            n_rep,
...
def test_dml_qte_coef(dml_qte_fixture):
    assert np.allclose(
        dml_qte_fixture["coef"], dml_qte_fixture["coef_manual"], rtol=1e-9, atol=1e-4
    )
...
def test_dml_qte_se(dml_qte_fixture):
    assert np.allclose(
        dml_qte_fixture["se"], dml_qte_fixture["se_manual"], rtol=1e-9, atol=1e-4
    )
...
def test_dml_qte_boot(dml_qte_fixture):
    for bootstrap in dml_qte_fixture["boot_methods"]:
        assert np.allclose(
            dml_qte_fixture["boot_qte_" + bootstrap],
...
def test_dml_qte_ci(dml_qte_fixture):
    assert np.allclose(
        dml_qte_fixture["ci"], dml_qte_fixture["ci_manual"], rtol=1e-9, atol=1e-4
    )
    for bootstrap in dml_qte_fixture["boot_methods"]:
        assert np.allclose(
            dml_qte_fixture["boot_ci_" + bootstrap],
...
def test_doubleml_qte_exceptions():
    ...
    msg = (
        r"Sample splitting not specified. Draw samples via .draw_sample splitting\(\). "
