...
def fit_qte(
    ...
    for i_quant in range(n_quantiles):
        se[i_quant] = np.sqrt(
            np.median(
                np.power(ses[i_quant, :], 2) * n_obs
                + np.power(qtes[i_quant, :] - qte[i_quant], 2)
            )
            / n_obs
        )
...
def boot_qte(
    ...
    for i_rep in range(n_rep):
        ...
        for i_quant in range(n_quantiles):
            ...
            boot_qte[i_quant, i_start:i_end] = (
                np.matmul(weights, scaled_scores[:, i_quant, i_rep]) / n_obs
            )
            boot_t_stat[i_quant, i_start:i_end] = np.matmul(
                weights, scaled_scores[:, i_quant, i_rep]
...
def confint_qte(coef, se, quantiles, boot_t_stat=None, joint=True, level=0.95):
    ...
    df_ci = pd.DataFrame(
        ci, columns=["{:.1f} %".format(i * 100) for i in ab], index=quantiles
    )
    return df_ci
