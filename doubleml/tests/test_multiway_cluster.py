...
from ._utils_cluster import (
    DoubleMLMultiwayResampling,
    var_one_way_cluster,
    est_one_way_cluster_dml2,
    est_two_way_cluster_dml2,
    var_two_way_cluster,
)
...
def dml_pliv_multiway_cluster_old_vs_new_fixture(generate_data_iv, learner):
    ...
    np.random.seed(3141)
    dml_pliv_obj_cluster = dml.DoubleMLPLIV(
    ...
    res_dict = {"coef": dml_pliv_obj.coef, "coef_manual": dml_pliv_obj_cluster.coef}

    return res_dict


@pytest.mark.ci
def test_dml_pliv_multiway_cluster_old_vs_new_coef(
    dml_pliv_multiway_cluster_old_vs_new_fixture,
):
    assert math.isclose(
        dml_pliv_multiway_cluster_old_vs_new_fixture["coef"],
...
def dml_pliv_multiway_cluster_fixture(generate_data_iv, learner, score, dml_procedure):
    ...
    for i_rep in range(n_rep):
        ...
        (
            ...
            d_minus_r_hat,
            y_minus_g_hat,
        ) = compute_pliv_residuals(y, d, z, l_hat, m_hat, r_hat, g_hat, smpls_one_split)
    ...
    se = np.sqrt(
        np.median(np.power(ses, 2) * var_scaling_factor + np.power(thetas - theta, 2))
        / var_scaling_factor
    )
...
def test_dml_pliv_multiway_cluster_coef(dml_pliv_multiway_cluster_fixture):
    assert math.isclose(
        dml_pliv_multiway_cluster_fixture["coef"],
...
def test_dml_pliv_multiway_cluster_se(dml_pliv_multiway_cluster_fixture):
    assert math.isclose(
        dml_pliv_multiway_cluster_fixture["se"],
...
def dml_pliv_oneway_cluster_fixture(generate_data_iv, learner, score, dml_procedure):
    ...
    var = var_one_way_cluster(
        psi, psi_a, obj_dml_oneway_cluster_data.cluster_vars[:, 0], smpls_one_split
    )
    se = np.sqrt(var)
...
def test_dml_pliv_oneway_cluster_coef(dml_pliv_oneway_cluster_fixture):
    assert math.isclose(
        dml_pliv_oneway_cluster_fixture["coef"],
...
def test_dml_pliv_oneway_cluster_se(dml_pliv_oneway_cluster_fixture):
    assert math.isclose(
        dml_pliv_oneway_cluster_fixture["se"],
...
def dml_plr_cluster_with_index(generate_data1, learner, dml_procedure):
    ...
    np.random.seed(3141)
    dml_plr_cluster_obj = dml.DoubleMLPLR(
...
def test_dml_plr_cluster_with_index_coef(dml_plr_cluster_with_index):
    assert math.isclose(
        dml_plr_cluster_with_index["coef"],
