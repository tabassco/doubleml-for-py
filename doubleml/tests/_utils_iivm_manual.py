...
def fit_iivm(
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            thetas[i_rep], ses[i_rep] = iivm_dml1(
                y,
                x,
                d,
                z,
            ...
            thetas[i_rep], ses[i_rep] = iivm_dml2(
                y,
                x,
                d,
                z,
...
def fit_nuisance_iivm(
    ...
    if is_classifier(learner_g):
        g_hat0_list = fit_predict_proba(
            y, x, ml_g0, g0_params, smpls, train_cond=train_cond0
        )
    else:
    ...
    if is_classifier(learner_g):
        g_hat1_list = fit_predict_proba(
            y, x, ml_g1, g1_params, smpls, train_cond=train_cond1
        )
    else:
    ...
    m_hat_list = fit_predict_proba(
        z, x, ml_m, m_params, smpls, trimming_threshold=trimming_threshold
    )

    ml_r0 = clone(learner_r)
    if always_takers:
        r_hat0_list = fit_predict_proba(
            d, x, ml_r0, r0_params, smpls, train_cond=train_cond0
        )
    else:
        r_hat0_list = []
    ...
    if never_takers:
        r_hat1_list = fit_predict_proba(
            d, x, ml_r1, r1_params, smpls, train_cond=train_cond1
        )
    else:
        r_hat1_list = []
        for _, test_index in smpls:
...
def tune_nuisance_iivm(
    ...
    train_cond0 = np.where(z == 0)[0]
    g0_tune_res = tune_grid_search(
        y, x, ml_g, smpls, param_grid_g, n_folds_tune, train_cond=train_cond0
    )

    train_cond1 = np.where(z == 1)[0]
    g1_tune_res = tune_grid_search(
        y, x, ml_g, smpls, param_grid_g, n_folds_tune, train_cond=train_cond1
    )

    m_tune_res = tune_grid_search(z, x, ml_m, smpls, param_grid_m, n_folds_tune)

    if always_takers:
        r0_tune_res = tune_grid_search(
            d, x, ml_r, smpls, param_grid_r, n_folds_tune, train_cond=train_cond0
        )
    ...
    if never_takers:
        r1_tune_res = tune_grid_search(
            d, x, ml_r, smpls, param_grid_r, n_folds_tune, train_cond=train_cond1
        )
...
def iivm_dml2(
    ...
    theta_hat = iivm_orth(
        g_hat0, g_hat1, m_hat, r_hat0, r_hat1, u_hat0, u_hat1, w_hat0, w_hat1, z, score
    )
    se = np.sqrt(
        var_iivm(
            theta_hat,
...
def boot_iivm_single_split(
    ...
    if apply_cross_fitting:
        ...
        J = np.mean(
            -(
                ...
                - r_hat0[test_index]
                + np.divide(
                    np.multiply(z[test_index], w_hat1[test_index]), m_hat[test_index]
                )
                - np.divide(
                    np.multiply(1.0 - z[test_index], w_hat0[test_index]),
                    1.0 - m_hat[test_index],
                )
            )
        )
    ...
    boot_theta, boot_t_stat = boot_manual(
        psi, J, smpls, se, weights, n_rep, apply_cross_fitting
    )
