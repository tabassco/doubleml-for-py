...
@pytest.fixture(
    ...
    params=[
        [
            XGBRegressor(
                n_jobs=1, objective="reg:squarederror", eta=0.1, n_estimators=10
            ),
            XGBClassifier(
...
@pytest.fixture(
    scope="module",
    params=[[LinearRegression(), LogisticRegression(solver="lbfgs", max_iter=250)]],
)
...
def dml_irm_w_missing_fixture(
    generate_data_irm_w_missings,
    learner_xgboost,
    score,
    dml_procedure,
    normalize_ipw,
...
def test_dml_irm_w_missing_coef(dml_irm_w_missing_fixture):
    assert math.isclose(
        dml_irm_w_missing_fixture["coef"],
...
def test_dml_irm_w_missing_boot(dml_irm_w_missing_fixture):
    for bootstrap in dml_irm_w_missing_fixture["boot_methods"]:
        assert np.allclose(
            dml_irm_w_missing_fixture["boot_coef" + bootstrap],
