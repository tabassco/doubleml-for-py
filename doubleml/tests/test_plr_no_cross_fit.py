...
def test_dml_plr_no_cross_fit_coef(dml_plr_no_cross_fit_fixture):
    assert math.isclose(
        dml_plr_no_cross_fit_fixture["coef"],
...
def test_dml_plr_no_cross_fit_se(dml_plr_no_cross_fit_fixture):
    assert math.isclose(
        dml_plr_no_cross_fit_fixture["se"],
...
def test_dml_plr_no_cross_fit_boot(dml_plr_no_cross_fit_fixture):
    for bootstrap in dml_plr_no_cross_fit_fixture["boot_methods"]:
        assert np.allclose(
            dml_plr_no_cross_fit_fixture["boot_coef" + bootstrap],
...
@pytest.fixture(scope="module", params=[10, 13])
def n_rep(request):
    return request.param
...
def dml_plr_rep_no_cross_fit_fixture(generate_data1, learner, score, n_rep):
    ...
    for i_rep in range(n_rep):
        ...
        l_hat, m_hat, g_hat = fit_nuisance_plr(
            y, x, d, _clone(learner), _clone(learner), _clone(learner), smpls
        )
        ...
        thetas[i_rep], ses[i_rep] = plr_dml1(
            y, x, d, all_l_hat[i_rep], all_m_hat[i_rep], all_g_hat[i_rep], smpls, score
        )

    res_manual = np.median(thetas)
    se_manual = np.sqrt(
        np.median(
            np.power(ses, 2) * len(smpls[0][1]) + np.power(thetas - res_manual, 2)
        )
        / len(smpls[0][1])
    )
...
def test_dml_plr_rep_no_cross_fit_coef(dml_plr_rep_no_cross_fit_fixture):
    assert math.isclose(
        dml_plr_rep_no_cross_fit_fixture["coef"],
...
def test_dml_plr_rep_no_cross_fit_se(dml_plr_rep_no_cross_fit_fixture):
    assert math.isclose(
        dml_plr_rep_no_cross_fit_fixture["se"],
...
def test_dml_plr_rep_no_cross_fit_boot(dml_plr_rep_no_cross_fit_fixture):
    for bootstrap in dml_plr_rep_no_cross_fit_fixture["boot_methods"]:
        assert np.allclose(
            dml_plr_rep_no_cross_fit_fixture["boot_coef" + bootstrap],
            dml_plr_rep_no_cross_fit_fixture["boot_coef" + bootstrap + "_manual"],
            rtol=1e-9,
            atol=1e-4,
        )
        assert np.allclose(
            dml_plr_rep_no_cross_fit_fixture["boot_t_stat" + bootstrap],
...
def dml_plr_no_cross_fit_tune_fixture(generate_data1, learner, score, tune_on_folds):
    par_grid = {
        "ml_l": {"alpha": np.linspace(0.05, 0.95, 7)},
        "ml_m": {"alpha": np.linspace(0.05, 0.95, 7)},
    }
    if score == "IV-type":
        par_grid["ml_g"] = {"alpha": np.linspace(0.05, 0.95, 7)}
    n_folds_tune = 3
    ...
    _ = dml_plr_obj.tune(
        par_grid, tune_on_folds=tune_on_folds, n_folds_tune=n_folds_tune
    )
...
def test_dml_plr_no_cross_fit_tune_coef(dml_plr_no_cross_fit_tune_fixture):
    assert math.isclose(
        dml_plr_no_cross_fit_tune_fixture["coef"],
...
def test_dml_plr_no_cross_fit_tune_se(dml_plr_no_cross_fit_tune_fixture):
    assert math.isclose(
        dml_plr_no_cross_fit_tune_fixture["se"],
...
def test_dml_plr_no_cross_fit_tune_boot(dml_plr_no_cross_fit_tune_fixture):
    for bootstrap in dml_plr_no_cross_fit_tune_fixture["boot_methods"]:
        assert np.allclose(
            dml_plr_no_cross_fit_tune_fixture["boot_coef" + bootstrap],
            dml_plr_no_cross_fit_tune_fixture["boot_coef" + bootstrap + "_manual"],
            rtol=1e-9,
            atol=1e-4,
        )
        assert np.allclose(
            dml_plr_no_cross_fit_tune_fixture["boot_t_stat" + bootstrap],
