...
def fit_did_cs(
    ...
    se = np.sqrt(
        np.median(np.power(ses, 2) * n_obs + np.power(thetas - theta, 2)) / n_obs
    )
...
def fit_nuisance_did_cs(
    ...
    g_hat_d0_t0_list = fit_predict(
        y, x, ml_g_d0_t0, g_d0_t0_params, smpls, train_cond=train_cond_d0_t0
    )

    train_cond_d0_t1 = np.intersect1d(np.where(d == 0)[0], np.where(t == 1)[0])
    g_hat_d0_t1_list = fit_predict(
        y, x, ml_g_d0_t1, g_d0_t1_params, smpls, train_cond=train_cond_d0_t1
    )

    train_cond_d1_t0 = np.intersect1d(np.where(d == 1)[0], np.where(t == 0)[0])
    g_hat_d1_t0_list = fit_predict(
        y, x, ml_g_d1_t0, g_d1_t0_params, smpls, train_cond=train_cond_d1_t0
    )

    train_cond_d1_t1 = np.intersect1d(np.where(d == 1)[0], np.where(t == 1)[0])
    g_hat_d1_t1_list = fit_predict(
        y, x, ml_g_d1_t1, g_d1_t1_params, smpls, train_cond=train_cond_d1_t1
    )
    if score == "observational":
        ml_m = clone(learner_m)
        m_hat_list = fit_predict_proba(
            d, x, ml_m, m_params, smpls, trimming_threshold=trimming_threshold
        )
    else:
    ...
    if score == "observational":
        if in_sample_normalization:
            ...
            weight_resid_d1_t0 = -1.0 * np.divide(
                np.multiply(d, 1.0 - t), np.mean(np.multiply(d, 1.0 - t))
            )
            ...
            weight_resid_d0_t1 = -1.0 * np.divide(
                unscaled_d0_t1, np.mean(unscaled_d0_t1)
            )
            ...
            weight_resid_d1_t1 = np.divide(
                np.multiply(d, t), np.multiply(p_hat, lambda_hat)
            )
            weight_resid_d1_t0 = -1.0 * np.divide(
            ...
            weight_resid_d0_t1 = -1.0 * np.multiply(
                np.divide(np.multiply(1.0 - d, t), np.multiply(p_hat, lambda_hat)),
                prop_weighting,
            )
        ...
        if in_sample_normalization:
            ...
            weight_resid_d0_t1 = -1.0 * np.divide(
                np.multiply(1.0 - d, t), np.mean(np.multiply(1.0 - d, t))
            )
            weight_resid_d0_t0 = np.divide(
                np.multiply(1.0 - d, 1.0 - t), np.mean(np.multiply(1.0 - d, 1.0 - t))
            )
        else:
            weight_psi_a = np.ones_like(d)
            weight_g_d1_t1 = weight_psi_a
            ...
            weight_resid_d0_t1 = -1.0 * np.divide(
                np.multiply(1.0 - d, t), np.multiply(1.0 - p_hat, lambda_hat)
            )
            weight_resid_d0_t0 = np.divide(
                np.multiply(1.0 - d, 1.0 - t),
...
def tune_nuisance_did_cs(
    y, x, d, t, ml_g, ml_m, smpls, score, n_folds_tune, param_grid_g, param_grid_m
):
    ...
    g_d0_t0_tune_res = tune_grid_search(
        y, x, ml_g, smpls, param_grid_g, n_folds_tune, train_cond=smpls_d0_t0
    )
    g_d0_t1_tune_res = tune_grid_search(
        y, x, ml_g, smpls, param_grid_g, n_folds_tune, train_cond=smpls_d0_t1
    )
    g_d1_t0_tune_res = tune_grid_search(
        y, x, ml_g, smpls, param_grid_g, n_folds_tune, train_cond=smpls_d1_t0
    )
    g_d1_t1_tune_res = tune_grid_search(
        y, x, ml_g, smpls, param_grid_g, n_folds_tune, train_cond=smpls_d1_t1
    )
...
def fit_sensitivity_elements_did_cs(
    y, d, t, all_coef, predictions, score, in_sample_normalization, n_rep
):
    n_treat = 1
    n_obs = len(y)
        ...
        if score == "observational":
            ...
            if in_sample_normalization:
                ...
                rr = (
                    ...
                    - np.multiply(
                        np.divide(d0t1, np.multiply(p_hat, lambda_hat)),
                        propensity_weight_d0,
                    )
                    + np.multiply(
                        np.divide(d0t0, np.multiply(p_hat, 1.0 - lambda_hat)),
                        propensity_weight_d0,
                    )
                )
        else:
