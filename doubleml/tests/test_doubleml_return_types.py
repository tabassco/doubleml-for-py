...
(x, y, d, t) = make_did_SZ2020(
    n_obs=n_obs, cross_sectional_data=True, return_type="array"
)
...
dml_iivm = DoubleMLIIVM(
    dml_data_iivm, Lasso(), LogisticRegression(), LogisticRegression()
)
dml_pliv_cluster = DoubleMLPLIV(dml_cluster_data_pliv, Lasso(), Lasso(), Lasso())
dml_cvar = DoubleMLCVAR(
    dml_data_irm, ml_g=RandomForestRegressor(), ml_m=RandomForestClassifier()
)
dml_pq = DoubleMLPQ(
    dml_data_irm, ml_g=RandomForestClassifier(), ml_m=RandomForestClassifier()
)
dml_lpq = DoubleMLLPQ(
    dml_data_iivm, ml_g=RandomForestClassifier(), ml_m=RandomForestClassifier()
)
dml_did = DoubleMLDID(dml_data_did, Lasso(), LogisticRegression())
dml_did_binary_outcome = DoubleMLDID(
    dml_data_did_binary_outcome, LogisticRegression(), LogisticRegression()
)
dml_did_cs = DoubleMLDIDCS(dml_data_did_cs, Lasso(), LogisticRegression())
dml_did_cs_binary_outcome = DoubleMLDIDCS(
    dml_data_did_cs_binary_outcome, LogisticRegression(), LogisticRegression()
)
...
def test_return_types(dml_obj, cls):
    ...
    assert isinstance(
        dml_obj.__str__(), str
    )  # called again after fit, now with numbers
    assert isinstance(
        dml_obj.summary, pd.DataFrame
    )  # called again after fit, now with numbers
    if not dml_obj._is_cluster_data:
...
def test_property_types_and_shapes(dml_obj):
    ...
    all_tuple = all(
        [all([isinstance(tpl, tuple) for tpl in smpl]) for smpl in dml_obj.smpls]
    )
    assert all_tuple
...
def test_stored_models():
    ...
    assert np.all(
        [
            isinstance(mdl, plr_dml1.learner["ml_l"].__class__)
            for mdl in plr_dml1.models["ml_l"]["d"][0]
        ]
    )
    assert np.all(
        [
            isinstance(mdl, plr_dml1.learner["ml_m"].__class__)
            for mdl in plr_dml1.models["ml_m"]["d"][0]
        ]
    )
...
def test_sensitivity():
    ...
    assert isinstance(
        plr_dml1.sensitivity_plot(value="ci", benchmarks=benchmarks),
        plotly.graph_objs._figure.Figure,
    )
    assert isinstance(
        plr_dml1._calc_sensitivity_analysis(cf_y=0.03, cf_d=0.03, rho=1.0, level=0.95),
        dict,
    )
    assert isinstance(
        plr_dml1._calc_robustness_value(
            null_hypothesis=0.0, level=0.95, rho=1.0, idx_treatment=0
        ),
        tuple,
    )
    ...
    assert isinstance(
        irm_dml1.sensitivity_plot(value="ci", benchmarks=benchmarks),
        plotly.graph_objs._figure.Figure,
    )
    assert isinstance(
        irm_dml1._calc_sensitivity_analysis(cf_y=0.03, cf_d=0.03, rho=1.0, level=0.95),
        dict,
    )
    assert isinstance(
        irm_dml1._calc_robustness_value(
            null_hypothesis=0.0, level=0.95, rho=1.0, idx_treatment=0
        ),
        tuple,
    )
    ...
    assert isinstance(
        did_dml1.sensitivity_plot(value="ci", benchmarks=benchmarks),
        plotly.graph_objs._figure.Figure,
    )
    assert isinstance(
        did_dml1._calc_sensitivity_analysis(cf_y=0.03, cf_d=0.03, rho=1.0, level=0.95),
        dict,
    )
    assert isinstance(
        did_dml1._calc_robustness_value(
            null_hypothesis=0.0, level=0.95, rho=1.0, idx_treatment=0
        ),
        tuple,
    )
    ...
    assert isinstance(
        did_cs_dml1.sensitivity_plot(value="ci", benchmarks=benchmarks),
        plotly.graph_objs._figure.Figure,
    )
    assert isinstance(
        did_cs_dml1._calc_sensitivity_analysis(
            cf_y=0.03, cf_d=0.03, rho=1.0, level=0.95
        ),
        dict,
    )
    assert isinstance(
        did_cs_dml1._calc_robustness_value(
            null_hypothesis=0.0, level=0.95, rho=1.0, idx_treatment=0
        ),
        tuple,
    )
...
def test_policytree():
    ...
    predict_features = pd.DataFrame(
        np.random.normal(size=(5, 20)), columns=features.keys()
    )
