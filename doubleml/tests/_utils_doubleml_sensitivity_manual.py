...
def doubleml_sensitivity_manual(
    sensitivity_elements, all_coefs, psi, psi_deriv, cf_y, cf_d, rho, level
):
    ...
    all_theta_lower = all_coefs - np.multiply(
        np.transpose(np.squeeze(S, axis=0)), confounding_strength
    )
    all_theta_upper = all_coefs + np.multiply(
        np.transpose(np.squeeze(S, axis=0)), confounding_strength
    )
    ...
    all_sigma_lower = np.transpose(
        np.sqrt(np.divide(np.mean(np.square(psi_lower), axis=0), var_scaling_factor))
    )
    all_sigma_upper = np.transpose(
        np.sqrt(np.divide(np.mean(np.square(psi_upper), axis=0), var_scaling_factor))
    )

    theta_lower, sigma_lower = _aggregate_coefs_and_ses(
        all_theta_lower, all_sigma_lower, var_scaling_factor
    )
