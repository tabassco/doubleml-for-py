...
def fit_plr_multitreat(
    ...
    for i_d in range(n_d):
        ...
        se[i_d] = np.sqrt(
            np.median(np.power(se_vec, 2) * n_obs + np.power(theta_vec - theta[i_d], 2))
            / n_obs
        )
...
def fit_plr_single_split(
    ...
    if dml_procedure == "dml1":
        theta, se = plr_dml1(y, x, d, l_hat, m_hat, g_hat, smpls, score)
    else:
...
def fit_nuisance_plr(
    ...
    if fit_g:
        y_minus_l_hat, d_minus_m_hat, _ = compute_plr_residuals(
            y, d, l_hat, m_hat, [], smpls
        )
...
def tune_nuisance_plr(
    ...
    if tune_g:
        ...
        g_tune_res = tune_grid_search(
            y - theta_initial * d, x, ml_g, smpls, param_grid_g, n_folds_tune
        )
...
def plr_dml1(y, x, d, l_hat, m_hat, g_hat, smpls, score):
    thetas = np.zeros(len(smpls))
    n_obs = len(y)
    y_minus_l_hat, d_minus_m_hat, y_minus_g_hat = compute_plr_residuals(
        y, d, l_hat, m_hat, g_hat, smpls
    )

    for idx, (_, test_index) in enumerate(smpls):
        thetas[idx] = plr_orth(
            y_minus_l_hat[test_index],
            d_minus_m_hat[test_index],
            y_minus_g_hat[test_index],
            d[test_index],
            score,
        )
    ...
    if len(smpls) > 1:
        se = np.sqrt(
            var_plr(
                theta_hat, d, y_minus_l_hat, d_minus_m_hat, y_minus_g_hat, score, n_obs
            )
        )
    else:
        assert len(smpls) == 1
...
def var_plr(theta, d, y_minus_l_hat, d_minus_m_hat, y_minus_g_hat, score, n_obs):
    if score == "partialling out":
        var = (
            ...
            * np.mean(
                np.power(
                    np.multiply(y_minus_l_hat - d_minus_m_hat * theta, d_minus_m_hat), 2
                )
            )
        )
    else:
        ...
        var = (
            ...
            * np.mean(
                np.power(np.multiply(y_minus_g_hat - d * theta, d_minus_m_hat), 2)
            )
        )
...
def plr_orth(y_minus_l_hat, d_minus_m_hat, y_minus_g_hat, d, score):
    if score == "IV-type":
        res = np.mean(np.multiply(d_minus_m_hat, y_minus_g_hat)) / np.mean(
            np.multiply(d_minus_m_hat, d)
        )
    else:
...
def boot_plr_single_split(
    ...
    if apply_cross_fitting:
        ...
        if score == "partialling out":
            J = np.mean(
                -np.multiply(d_minus_m_hat[test_index], d_minus_m_hat[test_index])
            )
        else:
            assert score == "IV-type"
        ...
        for i_treat in range(n_treat):
            ...
            if score == "partialling out":
                ...
                sigma2_score_element = np.square(
                    y - l_hat - np.multiply(theta, d_tilde - m_hat)
                )
            else:
                assert score == "IV-type"
                g_hat = predictions["ml_g"][:, i_rep, i_treat]
                sigma2_score_element = np.square(
                    y - g_hat - np.multiply(theta, d_tilde)
                )

            sigma2[0, i_rep, i_treat] = np.mean(sigma2_score_element)
            psi_sigma2[:, i_rep, i_treat] = (
                sigma2_score_element - sigma2[0, i_rep, i_treat]
            )
