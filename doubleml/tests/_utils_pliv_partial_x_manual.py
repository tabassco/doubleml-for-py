...
def fit_pliv_partial_x(
    ...
    for i_rep in range(n_rep):
        ...
        if dml_procedure == "dml1":
            thetas[i_rep], ses[i_rep] = pliv_partial_x_dml1(
                y, x, d, z, l_hat, m_hat, r_hat, smpls, score
            )
        else:
            assert dml_procedure == "dml2"
            thetas[i_rep], ses[i_rep] = pliv_partial_x_dml2(
                y, x, d, z, l_hat, m_hat, r_hat, smpls, score
            )
...
def fit_nuisance_pliv_partial_x(
    y, x, d, z, ml_l, ml_m, ml_r, smpls, l_params=None, m_params=None, r_params=None
):
    assert z.ndim == 2
    ...
    r_hat_tilde = (
        ...
        .fit(z - m_hat_array, d - r_hat_array)
        .predict(z - m_hat_array)
    )

    return l_hat, r_hat, r_hat_tilde


def tune_nuisance_pliv_partial_x(
    y,
    x,
    d,
    z,
    ...
    for i_instr in range(z.shape[1]):
        m_tune_res.append(
            tune_grid_search(z[:, i_instr], x, ml_m, smpls, param_grid_m, n_folds_tune)
        )
    ...
    m_best_params = [
        [xx.best_params_ for xx in m_tune_res[i_instr]] for i_instr in range(z.shape[1])
    ]
...
def pliv_partial_x_dml1(y, x, d, z, l_hat, r_hat, r_hat_tilde, smpls, score):
    ...
    for idx, (_, test_index) in enumerate(smpls):
        thetas[idx] = pliv_partial_x_orth(
            u_hat[test_index],
            w_hat[test_index],
            r_hat_tilde[test_index],
            d[test_index],
            score,
        )
...
def pliv_partial_x_orth(u_hat, w_hat, r_hat_tilde, d, score):
    assert score == "partialling out"
    res = np.mean(np.multiply(r_hat_tilde, u_hat)) / np.mean(
        np.multiply(r_hat_tilde, w_hat)
    )
...
def boot_pliv_partial_x_single_split(
    theta, y, d, z, l_hat, r_hat, r_hat_tilde, smpls, score, se, weights, n_rep_boot
):
    assert score == "partialling out"
    u_hat, w_hat = compute_pliv_partial_x_residuals(y, d, l_hat, r_hat, smpls)
