...
class DoubleMLBaseData(ABC):
    ...
    def __init__(self, data):
        if not isinstance(data, pd.DataFrame):
            raise TypeError(
                "data must be of pd.DataFrame type. "
                f"{str(data)} of type {str(type(data))} was passed."
            )
        if not data.columns.is_unique:
            raise ValueError(
                "Invalid pd.DataFrame: " "Contains duplicate column names."
            )
        self._data = data
...
class DoubleMLData(DoubleMLBaseData):
    ...
    def from_arrays(
        ...
        x = check_array(
            x, ensure_2d=False, allow_nd=False, force_all_finite=force_all_x_finite
        )
        ...
        if (z is None) and (t is None):
            data = pd.DataFrame(
                np.column_stack((x, y, d)), columns=x_cols + [y_col] + d_cols
            )
    ...
    def x_cols(self, value):
        ...
        if value is not None:
            ...
            if not len(set(value)) == len(value):
                raise ValueError(
                    "Invalid covariates x_cols: " "Contains duplicate values."
                )
            if not set(value).issubset(set(self.all_variables)):
                raise ValueError(
                    "Invalid covariates x_cols. "
            ...
            if (self.z_cols is not None) & (self.t_col is not None):
                y_d_z_t = set.union(
                    {self.y_col}, set(self.d_cols), set(self.z_cols), {self.t_col}
                )
    ...
    def d_cols(self, value):
        ...
        if not len(set(value)) == len(value):
            raise ValueError(
                "Invalid treatment variable(s) d_cols: " "Contains duplicate values."
            )
        if not set(value).issubset(set(self.all_variables)):
            raise ValueError(
                "Invalid treatment variable(s) d_cols. "
    ...
    def y_col(self, value):
        ...
        if not isinstance(value, str):
            raise TypeError(
                "The outcome variable y_col must be of str type. "
                f"{str(value)} of type {str(type(value))} was passed."
            )
        if value not in self.all_variables:
            raise ValueError(
                "Invalid outcome variable y_col. " f"{value} is no data column."
            )
        self._y_col = value
        if reset_value:
            self._check_disjoint_sets()
    ...
    def z_cols(self, value):
        ...
        if value is not None:
            ...
            if not len(set(value)) == len(value):
                raise ValueError(
                    "Invalid instrumental variable(s) z_cols: "
    ...
    def t_col(self, value):
        ...
        if value is not None:
            ...
            if value not in self.all_variables:
                raise ValueError(
                    "Invalid time variable t_col. " f"{value} is no data column."
                )
            self._t_col = value
        else:
            self._t_col = None
    ...
    def use_other_treat_as_covariate(self, value):
        ...
        if not isinstance(value, bool):
            raise TypeError(
                "use_other_treat_as_covariate must be True or False. "
                f"Got {str(value)}."
            )
    ...
    def set_x_d(self, treatment_var):
        ...
        if treatment_var not in self.d_cols:
            raise ValueError(
                "Invalid treatment_var. " f"{treatment_var} is not in d_cols."
            )
    ...
    def _check_disjoint_sets_y_d_x_z_t(self):
        ...
        if not y_col_set.isdisjoint(x_cols_set):
            raise ValueError(
                f"{str(self.y_col)} cannot be set as outcome variable ``y_col`` and covariate in "
                "``x_cols``."
            )
        if not y_col_set.isdisjoint(d_cols_set):
            raise ValueError(
                f"{str(self.y_col)} cannot be set as outcome variable ``y_col`` and treatment variable in "
                "``d_cols``."
            )
        ...
        if self.z_cols is not None:
            ...
            if not x_cols_set.isdisjoint(z_cols_set):
                raise ValueError(
                    "At least one variable/column is set as covariate (``x_cols``) and instrumental "
                    "variable in ``z_cols``."
                )

        if not t_col_set.isdisjoint(x_cols_set):
            raise ValueError(
                f"{str(self.t_col)} cannot be set as time variable ``t_col`` and covariate in "
                "``x_cols``."
            )
        if not t_col_set.isdisjoint(d_cols_set):
            raise ValueError(
                f"{str(self.t_col)} cannot be set as time variable ``t_col`` and treatment variable in "
                "``d_cols``."
            )
        if not t_col_set.isdisjoint(y_col_set):
            raise ValueError(
                f"{str(self.t_col)} cannot be set as time variable ``t_col`` and outcome variable "
                "``y_col``."
            )
...
class DoubleMLClusterData(DoubleMLData):
    ...
    def from_arrays(
        ...
        dml_data = DoubleMLData.from_arrays(
            x, y, d, z, t, use_other_treat_as_covariate, force_all_x_finite
        )
        ...
        if cluster_vars.shape[1] == 1:
            ...
            cluster_cols = [
                f"cluster_var{i + 1}" for i in np.arange(cluster_vars.shape[1])
            ]

        data = pd.concat(
            (pd.DataFrame(cluster_vars, columns=cluster_cols), dml_data.data), axis=1
        )
    ...
    def cluster_cols(self, value):
        ...
        if not len(set(value)) == len(value):
            raise ValueError(
                "Invalid cluster variable(s) cluster_cols: "
                "Contains duplicate values."
            )
        if not set(value).issubset(set(self.all_variables)):
            raise ValueError(
    ...
    def x_cols(self, value):
        if value is not None:
            ...
            if (self.z_cols is not None) & (self.t_col is not None):
                ...
                y_d_t = set.union(
                    {self.y_col}, set(self.d_cols), {self.t_col}, set(self.cluster_cols)
                )
                x_cols = [col for col in self.data.columns if col not in y_d_t]
            else:
    ...
    def _check_disjoint_sets_cluster_cols(self):
        ...
        if not y_col_set.isdisjoint(cluster_cols_set):
            raise ValueError(
                f"{str(self.y_col)} cannot be set as outcome variable ``y_col`` and cluster "
        ...
        if not x_cols_set.isdisjoint(cluster_cols_set):
            raise ValueError(
                "At least one variable/column is set as covariate (``x_cols``) and cluster "
                "variable in ``cluster_cols``."
            )
        if self.z_cols is not None:
            z_cols_set = set(self.z_cols)
        ...
        if self.t_col is not None:
            if not t_col_set.isdisjoint(cluster_cols_set):
                raise ValueError(
                    f"{str(self.t_col)} cannot be set as time variable ``t_col`` and "
                    "cluster variable in ``cluster_cols``."
                )
