    ...
    def _est_coef(self, psi_elements, smpls=None, scaling_factor=None, inds=None):
        ...
        # for cluster and dml2 we need the smpls and the scaling factors
            ...
            for i_fold, (_, test_index) in enumerate(smpls):
                psi_a_subsample_mean += scaling_factor[i_fold] * np.sum(
                    psi_a[test_index]
                )
                psi_b_subsample_mean += scaling_factor[i_fold] * np.sum(
                    psi_b[test_index]
                )
    ...
    def _est_coef(self, psi_elements, smpls=None, scaling_factor=None, inds=None):
        ...
        if not bounded:
            root_res = root_scalar(
                score, x0=self._coef_start_val, fprime=score_deriv, method="newton"
            )
            ...
            signs_different, bracket_guess = _get_bracket_guess(
                score, self._coef_start_val, self._coef_bounds
            )
            ...
            if signs_different:
                ...
                signs_different, bracket_guess = _get_bracket_guess(
                    score, alt_coef_start, self._coef_bounds
                )

                if signs_different:
                    root_res = root_scalar(
                        score, bracket=bracket_guess, method="brentq"
                    )
                    theta_hat = root_res.root
                else:
                    score_val_sign = np.sign(score(alt_coef_start))
                    if score_val_sign > 0:
                        theta_hat, score_val, _ = fmin_l_bfgs_b(
                            score,
                            self._coef_start_val,
                            approx_grad=True,
                            bounds=[self._coef_bounds],
                        ...
                        theta_hat, neg_score_val, _ = fmin_l_bfgs_b(
                            neg_score,
                            self._coef_start_val,
                            approx_grad=True,
                            bounds=[self._coef_bounds],
