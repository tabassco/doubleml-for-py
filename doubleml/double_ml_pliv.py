...
class DoubleMLPLIV(LinearScoreMixin, DoubleML):
    ...
    **Partially linear IV regression (PLIV)** models take the form
    ...
    def __init__(
        ...
        if ml_g is not None:
            if (isinstance(self.score, str) & (self.score == "IV-type")) | callable(
                self.score
            ):
            ...
            raise ValueError(
                "For score = 'IV-type', learners ml_l, ml_m, ml_r and ml_g need to be specified."
            )
    ...
    def _check_score(self, score):
        if isinstance(score, str):
            ...
            if not callable(score):
                raise TypeError(
                    "score should be either a string or a callable. "
                    "%r was passed." % score
                )
        return score
    ...
    def _nuisance_est(self, smpls, n_jobs_cv, return_models=False):
        if self.partialX & (not self.partialZ):
            psi_elements, preds = self._nuisance_est_partial_x(
                smpls, n_jobs_cv, return_models
            )
        elif (not self.partialX) & self.partialZ:
            psi_elements, preds = self._nuisance_est_partial_z(
                smpls, n_jobs_cv, return_models
            )
        else:
            assert self.partialX & self.partialZ
            psi_elements, preds = self._nuisance_est_partial_xz(
                smpls, n_jobs_cv, return_models
            )
    ...
    def _nuisance_est_partial_x(self, smpls, n_jobs_cv, return_models=False):
        ...
        if self._dml_data.n_instr == 1:
            ...
            m_hat = {
                "preds": np.full(
                    (self._dml_data.n_obs, self._dml_data.n_instr), np.nan
                ),
            ...
            for i_instr in range(self._dml_data.n_instr):
                ...
                predictions["ml_m_" + self._dml_data.z_cols[i_instr]] = res_cv_predict[
                    "preds"
                ]
                targets["ml_m_" + self._dml_data.z_cols[i_instr]] = res_cv_predict[
                    "targets"
                ]
                models["ml_m_" + self._dml_data.z_cols[i_instr]] = res_cv_predict[
                    "models"
                ]
        ...
        if (self._dml_data.n_instr == 1) & ("ml_g" in self._learner):
            ...
            _check_finite_predictions(
                g_hat["preds"], self._learner["ml_g"], "ml_g", smpls
            )
        ...
        if isinstance(self.score, str):
            ...
            if self._dml_data.n_instr > 1:
                raise NotImplementedError(
                    "Callable score not implemented for DoubleMLPLIV.partialX "
    ...
    def _nuisance_est_partial_z(self, smpls, n_jobs_cv, return_models=False):
        y = self._dml_data.y
        xz, d = check_X_y(
            np.hstack((self._dml_data.x, self._dml_data.z)),
            self._dml_data.d,
        ...
        if isinstance(self.score, str):
            ...
            assert callable(self.score)
            raise NotImplementedError(
                "Callable score not implemented for DoubleMLPLIV.partialZ."
            )
    ...
    def _nuisance_est_partial_xz(self, smpls, n_jobs_cv, return_models=False):
        ...
        _check_finite_predictions(
            m_hat_tilde["preds"], self._learner["ml_r"], "ml_r", smpls
        )
        ...
        if isinstance(self.score, str):
            ...
            assert callable(self.score)
            raise NotImplementedError(
                "Callable score not implemented for DoubleMLPLIV.partialXZ."
            )
    ...
    def _nuisance_tuning_partial_x(
        ...
        if self._dml_data.n_instr > 1:
            ...
            for instr_var in self._dml_data.z_cols:
                params["ml_m_" + instr_var] = [
                    xx.best_params_ for xx in m_tune_res[instr_var]
                ]
            tune_res = {
                "l_tune": l_tune_res,
                "m_tune": m_tune_res,
