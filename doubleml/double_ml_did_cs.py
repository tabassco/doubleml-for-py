...
class DoubleMLDIDCS(LinearScoreMixin, DoubleML):
    ...
    def __init__(
        ...
        self._strata = self._dml_data.d.reshape(-1, 1) + 2 * self._dml_data.t.reshape(
            -1, 1
        )
        ...
        if self.score == "observational":
            if self.in_sample_normalization:
                ...
                weight_resid_d0_t1 = -1.0 * np.multiply(
                    np.divide(d0t1, np.multiply(p_hat, lambda_hat)), prop_weighting
                )
            ...
            if self.in_sample_normalization:
                ...
                weight_resid_d0_t1 = -1.0 * np.divide(
                    d0t1, np.multiply(1.0 - p_hat, lambda_hat)
                )
    ...
    def _sensitivity_element_est(self, preds):
        ...
        if self.score == "observational":
            ...
            if self.in_sample_normalization:
                ...
                m_alpha_1 = np.divide(1.0, lambda_hat) + np.divide(
                    1.0, 1.0 - lambda_hat
                )
                m_alpha = np.multiply(
                    np.divide(d, np.square(p_hat)),
                    np.multiply(m_alpha_1, 1.0 + propensity_weight_d0),
                )

                rr_1 = np.divide(t, np.multiply(p_hat, lambda_hat)) + np.divide(
