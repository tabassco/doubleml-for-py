...
def _check_integer(value, name, lower_bound=None, upper_bound=None):
    ...
    if lower_bound is not None:
        if value < lower_bound:
            raise ValueError(
                f"{name} must be larger or equal to {lower_bound}. "
                f"{str(value)} was passed."
            )
    if upper_bound is not None:
        if value > upper_bound:
            raise ValueError(
                f"{name} must be smaller or equal to {upper_bound}. "
                f"{str(value)} was passed."
            )
    return
...
def _check_smpl_split_tpl(tpl, n_obs, check_intersect=False):
    ...
    if check_intersect:
        if set(train_index) & set(test_index):
            raise ValueError(
                "Invalid sample split. Intersection of train and test indices is not empty."
            )

    if len(np.unique(train_index)) != len(train_index):
        raise ValueError(
            "Invalid sample split. Train indices contain non-unique entries."
        )
    if len(np.unique(test_index)) != len(test_index):
        raise ValueError(
            "Invalid sample split. Test indices contain non-unique entries."
        )
...
def _check_finite_predictions(preds, learner, learner_name, smpls):
    ...
    if not np.all(np.isfinite(preds[test_indices])):
        raise ValueError(
            f"Predictions from learner {str(learner)} for {learner_name} are not finite."
        )
    return
...
def _check_zero_one_treatment(obj_dml):
    ...
    zero_one_treat = np.all(
        (np.power(obj_dml._dml_data.d, 2) - obj_dml._dml_data.d) == 0
    )
...
def _check_treatment(treatment):
    if not isinstance(treatment, int):
        raise TypeError(
            "Treatment indicator has to be an integer. "
            + f"Object of type {str(type(treatment))} passed."
        )

    if (treatment != 0) & (treatment != 1):
        raise ValueError(
            "Treatment indicator has be either 0 or 1. "
...
def _check_quantile(quantile):
    if not isinstance(quantile, float):
        raise TypeError(
            "Quantile has to be a float. "
            + f"Object of type {str(type(quantile))} passed."
        )

    if (quantile <= 0) | (quantile >= 1):
        raise ValueError(
            "Quantile has be between 0 or 1. " + f"Quantile {str(quantile)} passed."
        )
    return
...
def _check_is_propensity(preds, learner, learner_name, smpls, eps=1e-12):
    ...
    if any((preds[test_indices] < eps) | (preds[test_indices] > 1 - eps)):
        warnings.warn(
            f"Propensity predictions from learner {str(learner)} for"
...
def _check_benchmarks(benchmarks):
    if benchmarks is not None:
        ...
        if not set(benchmarks.keys()) == {"cf_y", "cf_d", "name"}:
            raise ValueError(
                "benchmarks has to be a dictionary with keys cf_y, cf_d and name. "
        ...
        if not len(set(value_lengths)) == 1:
            raise ValueError(
                "benchmarks has to be a dictionary with values of same length. "
